...
# --us-only: only load US metadata (safe when you have your own projects). Can be passed when running via exec.
SEED_US_ONLY=""
[ "$1" = "--us-only" ] && SEED_US_ONLY="1"
[ "${SEED_DEMO_US_ONLY}" = "1" ] && SEED_US_ONLY="1"
[ "${SEED_DEMO_US_ONLY}" = "true" ] && SEED_US_ONLY="1"
...
# SEED_US_ONLY: only load US metadata (workbook/entries/revisions), do not touch pg-demo-db. Set by --us-only or SEED_DEMO_US_ONLY=1.
if [ -z "${SEED_US_ONLY}" ]; then
  ...
  echo "  [demo] US-only mode: skip pg-demo-db, only import us demo entries..."
fi
