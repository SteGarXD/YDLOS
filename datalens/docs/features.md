...
В интерфейсе дашбордов добавлена возможность скачать данные в Excel. Файл формируется для табличных данных. **Ограничения:** изображения (графики) в Excel не сохраняются, интерфейс может выдать ошибку; форматирование таблиц и сводной (цвета, заливка, итоги) пока не полностью сохраняется. Целевое состояние «идеального» экспорта (в т.ч. цвета и значения в таблицах/сводной, чарты в файле) описано в проектной документации: `docs/EXPORT-PDF-EXCEL-SPEC.md`.
...
Для графиков, которые основаны на D3 (HC="0") добавлена функция для сохранения изображения в PDF. **Ограничение:** экспорт в PDF пока не полноценен для всех типов виджетов (не все чарты, не дашборд целиком, не таблицы/сводная с цветами). Целевое состояние: `docs/EXPORT-PDF-EXCEL-SPEC.md`.
