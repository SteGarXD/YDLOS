...
class DatasetValidator(DatasetBaseWrapper):
    ...
    def _update_field(
        ...
        # YDL OS: upstream требовал value_constraint для string + template_enabled — не добавляем ошибку:
        # даты, списки через запятую и произвольные строки для TVF/SQL без доп. ограничений.
