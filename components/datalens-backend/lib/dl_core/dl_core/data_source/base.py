...
def _sql_quote_string(s: str) -> str:
    """Escape single quotes and wrap in single quotes for SQL string literal. YDL OS."""
    escaped = s.replace("'", "''")
    return f"'{escaped}'"


def _mssql_safe_datetime_str(s: str) -> str:
    """Convert ISO datetime (with T/Z) to format MSSQL accepts (space, no Z). YDL OS."""
    s = s.replace("T", " ")
    s = re.sub(r"\.\d+Z", "", s)  # .000Z .999Z etc.
    return s.split("+")[0].rstrip("Z")
...
class DataSourceRenderer:
    ...
    def _render_bi_value(self, value: BIValue) -> str:
        ...
        if isinstance(raw, str):
            # Строки вида _interval_...T...Z_...T...Z — даты для MSSQL (без T/Z)
            if "_interval_" in raw or ("T" in raw and ("Z" in raw or "+" in raw)):
                raw = _mssql_safe_datetime_str(raw)
            return _sql_quote_string(raw)
        if hasattr(raw, "isoformat"):  # date, datetime — формат для MSSQL (без T/Z)
            dt_str = raw.isoformat() if callable(getattr(raw, "isoformat")) else str(raw)
            return _sql_quote_string(_mssql_safe_datetime_str(dt_str))
...
class DataSource(metaclass=abc.ABCMeta):
    ...
    default_chunk_row_count: ClassVar[int] = 100_000
