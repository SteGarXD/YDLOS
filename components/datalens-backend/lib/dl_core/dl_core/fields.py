...
class RegexParameterValueConstraint(BaseParameterValueConstraint):
    ...
    def _is_valid(self, value: Any) -> bool:
        ...
        # YDL OS: backward compat — old default for string params was [a-zA-Z0-9]* or [a-zA-Z0-9]+; accept any string
        p = self.pattern.strip()
        if p in ("[a-zA-Z0-9]*", "[a-zA-Z0-9]+"):
            return True
        if re.match(r"^" + re.escape("[a-zA-Z0-9]*") + r"$", p) or re.match(r"^" + re.escape("[a-zA-Z0-9]+") + r"$", p):
            return True
...
class DefaultParameterValueConstraint(RegexParameterValueConstraint):
    """Default constraint for string parameters. Accepts any string (dates, comma-separated lists, etc.)."""
    type: ParameterValueConstraintType = attr.ib(default=ParameterValueConstraintType.default)
    pattern: str = attr.ib(init=False, default=".*")
