...
class DatasetComponentAccessor:
    ...
    def get_parameter_values(self) -> dict[str, BIValue]:
        ...
        for field in self._dataset.result_schema.fields:
            # YDL OS: любой параметр с default попадает в словарь подстановки {{title}} в SQL.
            # Не требуем только template_enabled — иначе при рассинхроне UI/батча получаем Key ds not found in {}.
            if field.calc_mode != CalcMode.parameter or field.default_value is None:
                continue

            # YDL OS: accept parameter default values (dates, comma-separated lists for TVF) even if constraint fails
    ...
    def get_parameter_values_from_specs(
        ...
        for parameter_value_spec in parameter_value_specs:
            ...
            # YDL OS: accept parameter values (dates, comma-separated lists for TVF) even if constraint fails
