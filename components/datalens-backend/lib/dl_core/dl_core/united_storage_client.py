...
class USAuthContextRegular(USAuthContextBase):
    ...
    # When set (e.g. from USMFactory in data-api), sent on /v1 US calls alongside user RPC auth.
    us_master_token: str | None = attr.ib(default=None, kw_only=True, repr=False)
    ...
    def get_outbound_headers(self, include_tenancy: bool = True) -> dict[DLHeaders, str]:
        ...
        if self.us_master_token is not None:
            result[DLHeadersCommon.US_MASTER_TOKEN] = self.us_master_token
        return result
