...
class FormulaCompiler:
    ...
    def _generate_formula_obj_for_parameter_field(self, field: BIField) -> formula_nodes.Formula:
        ...
        # YDL OS: не валидируем value_constraint здесь — те же даты/списки через запятую, что и в
        # DatasetComponentAccessor.get_parameter_values (validateDataset / компиляция должны совпадать с превью).
