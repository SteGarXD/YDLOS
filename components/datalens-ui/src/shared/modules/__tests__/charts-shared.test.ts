...
describe('normalizeIntervalValueForMssqlScalar', () => {
    const interval =
        '__interval_2023-05-04T00:00:00.000Z_2023-05-26T23:59:59.999Z';

    it('maps dta1 to start, dta2 to end (dash alias same interval)', () => {
        expect(normalizeIntervalValueForMssqlScalar(interval, 'dta1')).toBe(
            '2023-05-04 00:00:00',
        );
        expect(normalizeIntervalValueForMssqlScalar(interval, 'dta2')).toBe(
            '2023-05-26 23:59:59',
        );
    });

    it('defaults to start for unrelated param keys (backward compatible)', () => {
        expect(normalizeIntervalValueForMssqlScalar(interval, 'flights')).toBe(
            '2023-05-04 00:00:00',
        );
    });
});
