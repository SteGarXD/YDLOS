export const opensourceEndpoints = {
    development: {
        api: {
            // Порты как в datalens/docker-compose.demo.yaml: us 8030, control-api 8010, data-api 8020
            us: process.env.US_ENDPOINT || 'http://localhost:8030',
    ...
    production: {
        api: {
            ...
            auth: process.env.AUTH_ENDPOINT || 'http://us-auth:80',
            s3Proxy: process.env.S3_PROXY_ENDPOINT || '',
        },
        ui: {
            gateway: process.env.UI_GATEWAY_ENDPOINT || '/gateway',
