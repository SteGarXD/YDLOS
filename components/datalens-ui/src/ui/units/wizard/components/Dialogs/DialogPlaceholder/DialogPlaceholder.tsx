...
import {
    ...
    AXIS_ORDER_RADIO_BUTTON_OPTIONS,
...
class DialogPlaceholder extends React.PureComponent<Props, State> {
    ...
    renderAxisOrderSettings() {
        const {item: placeholder} = this.props;
        const {axisOrder = SETTINGS.AXIS_ORDER.ASC} = this.state.settings;

        if (placeholder.id !== PlaceholderId.X) {
            return null;
        }

        return (
            <DialogPlaceholderRow
                title={i18n('wizard', 'label_axis-order')}
                setting={
                    <DialogRadioButtons
                        items={AXIS_ORDER_RADIO_BUTTON_OPTIONS}
                        value={axisOrder}
                        onUpdate={this.handleAxisOrderUpdate}
                        disabled={this.isAxisHidden()}
                    />
                }
            />
    ...
    handleAxisOrderUpdate = (axisOrder: string) => {
        this.setState({settings: {...this.state.settings, axisOrder}});
    };
}
