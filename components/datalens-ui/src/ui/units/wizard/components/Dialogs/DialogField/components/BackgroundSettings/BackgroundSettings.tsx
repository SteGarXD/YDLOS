...
import type {ClientChartsConfig, Field, NestedPartial, TableFieldBackgroundSettings} from 'shared';
import {
...
type Props = {
    onUpdate: (
        update:
            | NestedPartial<TableFieldBackgroundSettings, 'settings'>
            | Partial<Pick<TableFieldBackgroundSettings, 'cellStylePreset'>>,
    ) => void;
...
export const BackgroundSettings: React.FC<Props> = (props) => {
    ...
    const isPivotMeasures =
        (visualization.id as WizardVisualizationId) === WizardVisualizationId.PivotTable &&
        placeholderId === PlaceholderId.Measures;
    ...
    const cellStylePreset = state.cellStylePreset ?? '';

    return (
        <div className={b()}>
            {isPivotMeasures && (
                <DialogFieldRow
                    title={i18n('wizard', 'label_pivot_cell_style')}
                    setting={
                        <DialogFieldSelect
                            placeholder={i18n('wizard', 'label_none')}
                            options={[
                                {
                                    value: '',
                                    content: i18n('wizard', 'label_none'),
                                    qa: 'background-cell-preset-none',
                                },
                                {
                                    value: 'trafficLight',
                                    content: i18n('wizard', 'label_pivot_cell_style_traffic_light'),
                                    qa: 'background-cell-preset-traffic',
                                },
                                {
                                    value: 'turquoise',
                                    content: i18n('wizard', 'label_pivot_cell_style_turquoise'),
                                    qa: 'background-cell-preset-turquoise',
                                },
                            ]}
                            value={cellStylePreset}
                            onUpdate={(value) =>
                                onUpdate({
                                    cellStylePreset:
                                        (value as '' | 'trafficLight' | 'turquoise') || undefined,
                                })
                            }
                        />
                    }
