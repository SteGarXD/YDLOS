...
export interface GradientState {
    ...
    /** Без градиента: три дискретных цвета по порогам (только для 3-point). */
    useGradient?: boolean;
    discreteColorLow?: string;
    discreteColorMid?: string;
    discreteColorHigh?: string;
}
...
export function prepareDialogColorState(props: {
    ...
    return function (dispatch: WizardDispatch, getState: () => DatalensGlobalState) {
        ...
        const gradientState: GradientState = {
            ...
            useGradient: colorsConfig?.useGradient ?? defaultGradientState.useGradient,
            discreteColorLow:
                colorsConfig?.discreteColorLow ?? defaultGradientState.discreteColorLow,
            discreteColorMid:
                colorsConfig?.discreteColorMid ?? defaultGradientState.discreteColorMid,
            discreteColorHigh:
                colorsConfig?.discreteColorHigh ?? defaultGradientState.discreteColorHigh,
        };
