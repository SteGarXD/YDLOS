...
const mockedGraphVisualization =
    mockedVisualizations.find(
        ([id]) => chartTypeByVisualizationId[id] === WizardType.GraphWizardNode,
    )?.[1] ?? mockedVisualizations[0][1];
...
describe('Preview reducer', () => {
    describe('UPDATE_PREVIEW', () => {
        ...
        it('Should disable pagination when datasets more then one', () => {
            const mockedVisualization = mockedGraphVisualization;
        ...
        it('Should update only extraSettings, datasets, dataset, configType, visualization props when disable pagination', () => {
            ...
            expect(updatedState).toMatchObject({
                ...
                configType: chartTypeByVisualizationId[mockedVisualization.id!],
                visualization: mockedVisualization,
            });
        });

        it('Should set loading state when all checks was falsy', () => {
            const mockedVisualizationWithItems = mockedGraphVisualization;
            ...
            expect(stateWithVisualization).toMatchObject({
                ...
                configType: chartTypeByVisualizationId[mockedVisualizationWithItems.id!],
            });
    ...
    describe('UPDATE_CLIENT_CHARTS_CONFIG', () => {
        const defaultUpdateClientChartsConfigState = {
            ...defaultPreviewState,
            configType: chartTypeByVisualizationId[mockedGraphVisualization.id!],
            visualization: mockedGraphVisualization,
        };
