...
// Highcharts в итоге не должен быть включён: ничего из Highcharts не должно быть доступно в списке типов визуализаций (только бесплатный стек: D3, ECharts и т.д.).
const HIGHCHARTS_DISABLED = true;

export function getAvailableVisualizations(options?: ChartkitGlobalSettings) {
    const {
        highcharts: {enabled: hcFromSettings = false} = {},
        yandexMap: {enabled: isYandexMapEnabled = false} = {},
    } = options || DL.CHARTKIT_SETTINGS;
    const isHighchartsEnabled = HIGHCHARTS_DISABLED ? false : hcFromSettings;
    ...
    // Не показывать в списке варианты с hidden: иначе один и тот же вид (например линейная диаграмма из HC и из D3) дублируется в выпадающем списке.
    return _.cloneDeep(
        items.filter((item) => item.enabled && !get(item.value, 'hidden')).map(({value}) => value),
    );
}
