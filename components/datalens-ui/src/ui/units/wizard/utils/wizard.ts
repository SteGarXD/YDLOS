...
/** Settings that only apply to pivot table; must not persist on flat table or other charts. */
export function omitPivotOnlyExtraSettings<T extends CommonSharedExtraSettings | undefined>(
    settings: T,
): T {
    if (!settings) {
        return settings;
    }
    const next = {...settings} as CommonSharedExtraSettings;
    delete next.pivotFallback;
    delete next.pivotInlineSort;
    return next as T;
}
