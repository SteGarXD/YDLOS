...
export const Signin = ({alternativeAuthOptions, logoIcon}: SigninProps) => {
    ...
    /* Логика платформы: logoIcon приоритетен; иначе ребрандинг (SVG) или дефолт — PNG в интерфейсе */
    const logo = logoIcon ?? (isEnabledRebranding ? updatedLogo : LOGO_PNG_URL);
    const isLogoPng = typeof logo === 'string';
    ...
    return (
        ...
        >
            ...
            >
                {isEnabledRebranding ? (
                    <Icon data={logo} width="100%" className={b('rebranding-logo')} />
                ) : (
                    <Flex direction="column" gap="2" alignItems="center" className={b('branding')}>
                        {isLogoPng ? (
                            <img src={logo} alt="" className={b('logo')} />
                        ) : (
                            <Icon size={280} data={logo} className={b('logo')} />
                        )}
                        <Text variant="header-1" className={b('title')}>
                            {(typeof window !== 'undefined' &&
                                (window as Window & {DL?: {serviceName?: string}}).DL
                                    ?.serviceName) ||
                                i18n('title_product')}
                        </Text>
                    </Flex>
