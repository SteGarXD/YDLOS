...
export const getDataForParamsChecking = (args: {
    ...
    /** innerForm (cache_ttl_mode и др.) + form — иначе часть полей уходит undefined → BI 400. */
    const merged: FormDict = {...innerForm, ...form};

    return params.reduce((acc, param) => {
        let value: FormDictValue | undefined = merged[param];

        if (param === FieldKey.Port && value !== undefined && value !== null && value !== '') {
        ...
        if (param === FieldKey.Password && typeof value === 'string' && value === '******') {
            return acc;
        }
