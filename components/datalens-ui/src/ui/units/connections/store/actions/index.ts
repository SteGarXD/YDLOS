...
export function checkConnection() {
    return async (dispatch: ConnectionsReduxDispatch, getState: GetState) => {
        ...
        const newConnection = newConnectionSelector(getState());
        ...
        /** id из getConnection; если нет — entryId (тот же UUID в US), иначе ошибочно уходит verifyConnectionParams. */
        const connectionIdRaw = connectionData[FieldKey.Id] ?? connectionData.id;
        const connectionId =
            (typeof connectionIdRaw === 'string' && connectionIdRaw) ||
            (typeof connectionIdRaw === 'number' && String(connectionIdRaw)) ||
            (!newConnection && entry?.entryId ? entry.entryId : undefined);
