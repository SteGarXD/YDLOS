...
export function App({...routeProps}: RouteComponentProps) {
    ...
    const locationChangeHandler = React.useCallback(
        async (data) => {
            ...
            // Read fresh id from the store: history.listen can run synchronously on history.push
            // before React re-renders, so a closure over useSelector(stateHashId) would be stale
            // and we'd wrongly dispatch setTabHashState → getDashState and overwrite client state.
            const currentStateHashId = selectStateHashId(store.getState());
            if (newStateHashId === currentStateHashId) {
                return;
            }

            dispatch(setTabHashState({tabId: newTabId, stateHashId: newStateHashId, entryId}));
        },
        [entryId, tabs, dispatch, store],
    );
