...
import {withChartWidgetRuntimeDefaultsInConfig} from '../../utils/chartWidgetDashDefaults';
...
class Body extends React.PureComponent<BodyProps, DashBodyState> {
    ...
    updateUrlHashState = debounce(async (data, tabId) => {
        ...
        try {
            ...
            /*
             * Debounce вызывает createDashState с снимком `data` на trailing edge, но ответ может прийти
             * после новых SET_HASH_STATE (ещё один «+» во время сетевого round-trip). Пуш старого hash
             * ломает синхрон URL↔дашборд и даёт схлопывание treeState на таблице.
             */
            if (!isEqual(data, this.props.hashStates)) {
                return;
            }
    ...
    /** Кэш withChartWidgetRuntimeDefaultsInConfig: один и тот же tabData + inner → тот же wrapped. */
    _dashKitRuntimeDefaultsCache: {
        tab: DashTab | null | undefined;
        inner: DashKitProps['config'];
        wrapped: DashKitProps['config'];
    } | null = null;
    ...
    onChange = ({
        ...
        // Deep-compare: DashKit may pass a new object reference with the same payload; reference
        // checks and "non-empty keys" skipped clearing filters (empty {}) and missed URL↔state sync.
        if (itemsStateAndParams === undefined) {
            if (config) {
                this.props.setCurrentTabData(config as unknown as DashTab);
            }
            return;
        }
        if (!isEqual(this.props.hashStates, itemsStateAndParams)) {
    ...
    private getConfig = () => {
        ...
        let inner: DashKitProps['config'];
        if (!DL.IS_MOBILE) {
            inner = tabDataConfig as DashKitProps['config'];
        } else {
            ...
            inner = this._memoizedOrderedConfig!.config as DashKitProps['config'];
        }

        const c = this._dashKitRuntimeDefaultsCache;
        if (c && c.tab === tabDataConfig && c.inner === inner) {
            return c.wrapped;
        }
        const wrapped = withChartWidgetRuntimeDefaultsInConfig(
            inner,
        ) as DashKitProps['config'];
        this._dashKitRuntimeDefaultsCache = {tab: tabDataConfig, inner, wrapped};
        return wrapped;
    };
            ...
            >
                <WidgetContextProvider onWidgetMountChange={this.itemAddHandler}>
                    {shouldRenderMobileMenu && this.props.entryId && (
                        <FloatingMobileMenuWithContext
                            entryId={this.props.entryId}
                            dashData={this.props.entry?.data}
                            hasFixedContent={
