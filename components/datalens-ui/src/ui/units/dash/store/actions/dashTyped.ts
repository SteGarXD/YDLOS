...
export const setPageTab = (tabId: string) => {
    return async function (dispatch: DashDispatch, getState: GetState) {
        ...
        /* После ошибки загрузки дашборда (500) или до loadDash entry может быть null — иначе crash */
        if (dash.entry?.entryId) {
            collectDashStats({
...
/** После изменения данных групп в редакторе — перезапрос loadedData у селекторов (списки < LIMIT). */
export const BUMP_FLIGHT_GROUPS_DATASET_REFRESH = Symbol('dash/BUMP_FLIGHT_GROUPS_DATASET_REFRESH');
export type BumpFlightGroupsDatasetRefreshAction = {
    type: typeof BUMP_FLIGHT_GROUPS_DATASET_REFRESH;
};
export const bumpFlightGroupsDatasetRefresh = (): BumpFlightGroupsDatasetRefreshAction => ({
    type: BUMP_FLIGHT_GROUPS_DATASET_REFRESH,
});
