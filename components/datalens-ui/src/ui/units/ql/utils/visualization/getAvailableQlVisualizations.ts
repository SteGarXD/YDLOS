...
// Highcharts не должен быть доступен: в списке только типы на бесплатном стеке (D3 и т.д.).
...
export function getAvailableQlVisualizations(
    ...
    const {highcharts: {enabled: hcFromSettings = false} = {}} = options || DL.CHARTKIT_SETTINGS;
    ...
    // YDL OS: Pivot (svodnaya) is not offered in QL — only in Wizard. Do not add PIVOT_TABLE_VISUALIZATION to items.
