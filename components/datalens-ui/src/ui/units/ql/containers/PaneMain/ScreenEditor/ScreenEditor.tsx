...
class ScreenEditor extends React.PureComponent<ScreenEditorInnerProps, ScreenEditorState> {
    ...
    render() {
        const {chartType, defaultPath, paneSize, entryDialoguesRef, entry} = this.props;

        const workbookId = entry?.workbookId || null;

        const valid = (entry?.permissions || {execute: true}).execute;
        ...
        return (
            chartType && (
                <div className={b()}>
                    ...
                    {edit == true && this.state.activeTab === 'paramsTab' && (
                        <TabParams></TabParams>
                    )}
