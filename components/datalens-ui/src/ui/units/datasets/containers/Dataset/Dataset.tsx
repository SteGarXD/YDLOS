...
class Dataset extends React.Component<Props, State> {
    ...
    componentDidMount() {
        ...
        if (isCreationProcess) {
            this.props.setDatasetCreationContext({
                workbookId: this.getWorkbookId() || null,
            });
            initializeDataset({connectionId, collectionId});
        } else if (datasetId) {
            // Сразу кладём workbook из URL в store — заголовок X-DL-Workbook-Id до ответа getDataset
            const wb = this.getWorkbookId();
            if (wb) {
                this.props.setDatasetCreationContext({workbookId: wb});
            }
