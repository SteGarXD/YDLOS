...
client.interceptors.request.use((config) => {
    ...
    // OpenSource/dev: preview and US RPC may fail with US.AUTH.UNAUTHORIZED_ACCESS without master token.
    if (DL.installationType === 'opensource' && !config.headers[US_MASTER_TOKEN_HEADER]) {
        config.headers[US_MASTER_TOKEN_HEADER] = process.env.US_MASTER_TOKEN || 'us-master-token';
    }
    return config;
});
