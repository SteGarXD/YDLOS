...
import type {ChartConfigWithShared} from './flatTableRowTreeMenuUtils';
...
export type MenuItemArgs = {
    ...
    /** Chart request props (e.g. `config.data.shared` for visualization flags). */
    propsData?: ChartConfigWithShared;
};
...
export const getNewWindowMenuItem = ({
    ...
    isVisible: () => {
        return customConfig?.isVisible ? customConfig.isVisible() : true;
    },
    action:
        customConfig?.action ||
...
export const getLinkMenuItem = (customConfig?: Partial<MenuItemConfig>): MenuItemConfig => ({
    ...
    isVisible: ({loadedData}: MenuItemArgs) => {
        return customConfig?.isVisible ? customConfig?.isVisible() : Boolean(loadedData?.type);
    },
    action:
        customConfig?.action ||
