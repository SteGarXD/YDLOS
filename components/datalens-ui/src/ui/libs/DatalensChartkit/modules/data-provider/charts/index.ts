...
class ChartsDataProvider implements DataProvider<ChartsProps, ChartsData, CancelTokenSource> {
    ...
    async makeActivityRequest({
        ...
        try {
            ...
            if (result.status === 424 || result.status === 427) {
                return null;
            }
    ...
    async makeRequest(requestOptions: EntityRequestOptions & {url?: string}) {
        ...
        return axiosInstance(
            this.prepareRequestConfig({
                ...
                ...requestOptions,
                headers,
                // 424/427 — движок чартов: зависимости/загрузка источника «ещё не готово» (см. charts-engine constants).
                validateStatus: (status: number) =>
                    (status >= 200 && status < 300) || status === 424 || status === 427,
            }),
        );
    }
    ...
    private processError(args: {
        ...
        // 424/427 — дашборд/источники: зависимости или fetching ещё не готовы (см. charts-engine statusCode).
        if (status === 424 || status === 427) {
            return null;
        }
    ...
    private async load({
        ...
        // YDL OS: передаём полный config в run при наличии (в т.ч. с дашборда), чтобы shared.visualization.placeholders содержал backgroundSettings для окраски ЗПК % / Млн. р
        const hasConfigData = Boolean(configData);
