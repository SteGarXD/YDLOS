    ...
    const instance: ConcurrencyManagerInstance = {
        ...
        shift: () => {
            if (instance.queue.length) {
                ...
                if (queued) {
                    // Всегда вызывать resolver: иначе при отмене до выхода из очереди промис axios
                    // «висит», init() в групповом селекторе не доходит до catch/SUCCESS → вечный PENDING.
