...
function prepareValues({widget, data, widgetType, extra, options = {}}) {
    ...
    switch (widgetType) {
        ...
        case WidgetKind.Table: {
            ...
            /** Цвета фона ячеек (cell.css с бэкенда) для ExcelJS на сервере */
            const cellStyles = [];
            ...
            const mergeFooterItogoLabel =
                footer.length >= 2 &&
                footer.every((row) => {
                    const v = row.cells?.[0]?.value;
                    return /^итого$/i.test(String(v ?? '').trim());
                });

            const xlsxFooterMeta =
                format === EXPORT_FORMATS.XLSX
                    ? {
                          bodyRowCount: rows.length,
                          footerRowCount: footer.length,
                          mergeFooterItogoLabel,
                      }
                    : undefined;
            ...
            return {
                ...
                cellStyles,
                /** Сырое дерево шапки для двух строк и merge в Excel (сводные) */
                tableHead: format === EXPORT_FORMATS.XLSX ? head : undefined,
                xlsxFooterMeta,
            };
        }
    }
