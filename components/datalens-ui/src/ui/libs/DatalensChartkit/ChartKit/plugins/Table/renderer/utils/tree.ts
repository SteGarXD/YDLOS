...
function safeParseTreePath(json: string): string[] | null {
    try {
...
export function getUpdatesTreeState(args: {cell: TableCommonCell; params: StringParams}) {
    ...
    const cellTreeNode = safeParseTreePath(treeNode);
    if (!cellTreeNode) {
        return null;
    }

    const treeState = normalizeFlatTableTreeStateList(
        ([] as string[]).concat(params.treeState as string[]).filter(Boolean) as string[],
    );

    if (treeState.some((state) => state === treeNode)) {
        return treeState.filter((state) => {
            const item = safeParseTreePath(state);
            return !(item && isSubarray(item, cellTreeNode));
        });
    }
