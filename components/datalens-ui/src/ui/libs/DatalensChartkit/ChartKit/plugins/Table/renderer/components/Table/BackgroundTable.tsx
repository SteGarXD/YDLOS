...
type Props = {
    ...
    data: TableViewData;
    /** Ссылка на widgetData.data: при смене (новый ответ) сбрасываем накопленные min-width, иначе merge «только растёт» даёт кадр с неверной вёрсткой */
    sourceData?: unknown;
    size: WidgetSizeType;
...
export const BackgroundTable = React.memo<Props>((props: Props) => {
    const {
        dimensions,
        data: {header, body, measurementFooter},
        sourceData,
        onChangeMinWidth,
        size,
        width,
    ...
    if (sourceDataRef.current !== sourceData) {
        ...
        <Portal>
            ...
            >
                <table
                    ...
                    ref={tableRef}
                    style={{
                        /* width: 100% в широком контейнере раздувает колонки при измерении — берём ширину по контенту */
                        width: width === 'auto' ? 'max-content' : width,
                    }}
                >
                    <TableHead rows={header.rows} />
                    <TableBody rows={body.rows} />
                    <TableFooter rows={measurementFooter.rows} />
                </table>
            </div>
        </Portal>
    );
