...
/** head/rows/footer required; optional server-only flags stay optional */
export type TableDataMapped = Required<Omit<TableData, 'flatTableRowTreeHasNestedSegments'>>;

export function mapTableData(data: TableData | undefined): TableDataMapped {
    ...
    if (newHead.length && rows.length) {
        // old pivot tables: только одна строка без ячеек — подменяем одной пустой строкой для отображения
        // не трогаем ответы backend-pivot (несколько строк), иначе все данные заменятся одной пустой строкой
        const firstRow = rows[0];
        if (rows.length === 1 && 'cells' in firstRow && !firstRow.cells?.length) {
