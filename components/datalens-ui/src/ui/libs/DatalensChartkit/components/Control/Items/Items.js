...
/** YCSelect сравнивает выбранное через Set.has(item.value); типы должны совпадать с параметрами (обычно строки). */
function toSelectItem({title, value: rawValue}) {
    const strVal = String(rawValue);
    return {
        value: strVal,
        title: title != null && title !== '' ? String(title) : strVal,
        key: strVal,
    };
}

function normalizeMultiselectPropValue(value) {
    if (value == null || value === '') {
        return [];
    }
...
function normalizeSinglePropValue(value) {
    if (value == null || value === '') {
        return '';
    }
    if (Array.isArray(value)) {
...
function BaseControlSelect({
    ...
    const [currentValue, setCurrentValue] = React.useState(() =>
        multiselect ? normalizeMultiselectPropValue(value) : normalizeSinglePropValue(value),
    );

    React.useEffect(() => {
        if (multiselect) {
            setCurrentValue(normalizeMultiselectPropValue(value));
        } else {
            setCurrentValue(normalizeSinglePropValue(value));
        }
    }, [value, multiselect]);

    const items = content
        // because the choice of such values leads to incorrect behavior
        .filter(({value: v}) => v !== null && v !== '' && v !== undefined)
        .map(({title, value: v}) => toSelectItem({title, value: v}));
    ...
    const wrappedOnChange = React.useCallback(
        (next) => {
            ...
            const wrappedValue =
                next === null || (Array.isArray(next) && !next.length) ? '' : next;
                ...
                const arr =
                    wrappedValue === ''
                        ...
                        : Array.isArray(wrappedValue)
                          ? [
                                ...new Set(
                                    wrappedValue.map((v) => String(v).trim()).filter(Boolean),
                                ),
                            ]
                          : normalizeMultiselectPropValue(wrappedValue);
                setCurrentValue(arr);
                return onChange(arr.length ? arr : '');
            }

            const single =
                wrappedValue === '' || wrappedValue == null
                    ? ''
                    : normalizeSinglePropValue(wrappedValue);
            setCurrentValue(single);
            return onChange(single);
        },
...
function BaseControlRangeDatepicker({
    ...
    const wrappedOnChange = React.useCallback(
        ({from, to}) => {
            ...
            // Month-granularity UX: one click should be enough.
            // If only "from" is selected, auto-complete interval to end of that month.
            const autoCompletedTo = resultFrom && !to ? getEndOfMonth(resultFrom) : to;
            const resultTo = autoCompletedTo === null ? '' : autoCompletedTo;
    ...
    return (
        <DatepickerControl
            ...
            label={labelInside ? label : innerLabel}
            disabled={disabled}
            dateGranularityEnabled={dateGranularityEnabled}
        />
    );
}
...
BaseControlRangeDatepicker.propTypes = {
    ...
    disabled: PropTypes.bool,
    accentType: PropTypes.string,
    dateGranularityEnabled: PropTypes.object,
};
