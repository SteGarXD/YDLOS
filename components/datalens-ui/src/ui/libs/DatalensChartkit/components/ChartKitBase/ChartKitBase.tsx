...
> {
    ...
    private get dataProviderProps() {
        ...
        const propsP = (this.props.params || {}) as StringParams;
        const runtimeP = (this.state.runtimeParams || {}) as StringParams;
        const mergedTreeState = mergeTableTreeStateForDataProvider(propsP, runtimeP);
        const paramsWithTree =
            mergedTreeState !== undefined
                ? {...resultParams, treeState: mergedTreeState}
                : resultParams;
        return {
            ...omit(this.props, DATA_PROVIDER_IRRELEVANT_PROPS_KEYS),
            params: paramsWithTree,
            ...
            try {
                ...
                if (this.mounted && loadedData !== null) {
                    ...
                    if (isSentDataEqualToStateData || this.changedByTabSelect) {
                        ...
                        /*
                         * One ChartKit instance = one widget. Runtime UI params must not carry table-tree state
                         * from another visualization: e.g. flat row-tree keys are meaningless for backend pivot
                         * and were corrupting layout after type switches in the wizard.
                         */
                        if (loadedData && (loadedData as TWidget).type === 'table') {
                            const tw = loadedData as TWidget & {
                                data?: {
                                    head?: TableHead[];
                            ...
                            if (
                                tableDataLooksLikeBackendPivot({
                                    ...
                                    flatTableRowTreeHasNestedSegments:
                                        tw.data?.flatTableRowTreeHasNestedSegments,
                                })
                            ) {
                                newState.runtimeParams = omit(
    ...
    private onChange = (
        ...
        if (data.type === 'PARAMS_CHANGED') {
            ...
            // Таблица шлёт PARAMS_CHANGED с callExternalOnChange=true. Без принудительного setState
            // ветка else не обновляет runtimeParams — treeState не попадает в state, merge в renderBody
            // отдаёт в виджет пустой treeState: одно раскрытие «съедается», следующие клики ломают дерево.
            const isTreeStateUpdate = Object.prototype.hasOwnProperty.call(
                data.data.params || {},
                'treeState',
            );
            ...
            if (needForceChange) {
                // Функциональное обновление: два PARAMS_CHANGED подряд (два + у дерева) иначе
                // второй раз читает устаревший runtimeParams и затирает treeState.
                this.setState(
                    (prev) => ({
                        runtimeParams: deepAssign(
                            {},
                            prev.runtimeParams,
                            data.data.params,
                            additionalParams,
                    ...
                    () => {
                        this.onExternalChangeCallback({
                            ...
                            params: this.state.runtimeParams,
                        });
                    },
                );
            } else {
    ...
    private renderBody() {
        ...
        if (loadedData && loadedData.type !== 'control') {
            let loadedDataForAdapter = loadedData;
            // Пока /api/run в полёте, treeState и др. лежат в runtimeParams; Table читает только data.params.
            // Без слияния второй клик по + видит устаревший treeState и затирает раскрытие (всё «схлопывается»).
            // Сводная (отдельная визуализация) не получает treeState от плоского дерева рейсов — см. очистку после load.
            if (
                loadedData.type === 'table' &&
                this.state.runtimeParams &&
                Object.keys(this.state.runtimeParams).length > 0
            ) {
                const tablePayload = loadedData as {
                    data?: {head?: TableHead[]; flatTableRowTreeHasNestedSegments?: boolean};
                };
                const runtimeToMerge = tableDataLooksLikeBackendPivot({
                    head: tablePayload.data?.head,
                    flatTableRowTreeHasNestedSegments:
                        tablePayload.data?.flatTableRowTreeHasNestedSegments,
                })
                    ? omit(this.state.runtimeParams, 'treeState')
                    : this.state.runtimeParams;
                loadedDataForAdapter = {
                    ...loadedData,
                    params: deepAssign({}, loadedData.params || {}, runtimeToMerge),
                };
            }
            ...
            return (
                <ChartKitAdapter
                    ...
                    loadedData={loadedDataForAdapter}
