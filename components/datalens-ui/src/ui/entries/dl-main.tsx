import {
    suppressExtensionTabMessagingUnhandledRejection,
    suppressResizeObserverLoopDevNoise,
} from 'ui/utils/suppressResizeObserverLoopDevNoise';
...
import 'ui/styles/opensource-chartkit-theme.scss';
...
function renderApp() {
    ...
    // Показ #root только после шрифтов + двух кадров — меньше FOUC/FOUT на /auth и везде.
    if (typeof document !== 'undefined') {
        const markReady = () => {
            requestAnimationFrame(() => {
                requestAnimationFrame(() => {
                    document.documentElement.classList.add('dl-react-ready');
                });
            });
        };
