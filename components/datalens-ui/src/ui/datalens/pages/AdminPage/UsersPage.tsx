...
const UsersTables = () => {
    ...
    const columns = [
        ...
        {id: 'c_project_name', name: i18n('project')},
        {
            ...
            name: i18n('action'),
            template: (item: any) => (
                <Button onClick={onEdit(item)} size="m">
                    {i18n('change')}
                </Button>
            ),
        },
    ...
    async function onSave(values: any) {
        const base_roles = roles
            .filter((item: any) => item.isbase && item.name != 'oidc')
            .map((item: any) => item.name);
        if (_.intersection(base_roles, values.c_claims).length == 0) {
            return setError(i18n('not_select_require_roles'));
        }
        if (!values.id) {
            const {err: errCreate0, data: dataCreate} = await Utils.universalService({
                action: 'datalens',
                method: 'create_user',
                data: [
                    {
                        ...
                        c_project_name:
                            values.c_project_name && Array.isArray(values.c_project_name)
                                ? values.c_project_name[0]
                                : null,
                    },
                ],
            });
            const errCreate = errCreate0 || dataCreate[0]?.message;
            if (errCreate) {
                return setError(errCreate);
            }
        } else {
            const {err: errUpdate0, data: dataUpdate} = await Utils.universalService({
                action: 'datalens',
                method: 'update_user',
                data: [
                    {
            ...
            const errUpdate = errUpdate0 || dataUpdate[0]?.message;
            if (errUpdate) {
                return setError(errUpdate);
            }

            const {err: roles_err0, data: roles_data} = await Utils.universalService({
                action: 'datalens',
                method: 'update_roles',
                data: [
                    {
            ...
            const roles_err = roles_err0 || roles_data[0]?.message;
            if (roles_err) {
                return setError(roles_err);
            }

            if (values.c_password) {
                const {err: password_err0, data: password_data} = await Utils.universalService({
                    action: 'datalens',
                ...
                const password_err = password_err0 || password_data[0]?.message;
                if (password_err) {
    ...
    async function loadUsers(value = '') {
        ...
        const users = await Utils.universalService({
            action: 'datalens',
            method: 'users',
            data: [params],
        });
    ...
    async function loadData() {
        const roles = await Utils.universalService({
            action: 'datalens',
            method: 'roles',
            data: [{}],
        });
        const projects = await Utils.universalService({
            action: 'datalens',
            method: 'projects',
            data: [{}],
        });
    ...
    return (
        <div className={b('wrapper')}>
            <Text className={b('title')} variant="header-1">
                {i18n('users')}
            </Text>
            <div className="alert alert-light" role="alert">
                {i18n('alert_message')}
            </div>
            <UsersPageToolbar onSearch={onSearch} onCreate={onCreate} onRefresh={onRefresh} />
            <div className={b('container')}>
                {loading ? (
                    <Loader className={b('table')} />
                ) : (
                    <Table
