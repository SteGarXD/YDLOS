...
const ProjectTables = () => {
    ...
    const columns = [
        ...
        {id: 'name', name: i18n('constant')},
        {
            id: 'isbase',
            name: i18n('is_base'),
            template: (item: any) => (item.isbase ? i18n('yes') : i18n('no')),
        },
        {
            id: 'action',
            name: i18n('action'),
    ...
    async function onSave(values: any) {
        const {err: err0, data} = await Utils.universalService({
            action: 'datalens',
            method: 'add_or_update_project',
            data: [{id: values.id, c_name: values.constant, c_description: values.name}],
        });
        const err = err0 || data[0]?.message;
        if (err) {
            return setError(err);
        }
        ...
        <div className={b('wrapper')}>
            ...
            <div className={b('container')}>
                {loading ? (
                    ...
                    <Table data={data} columns={columns} className={b('table')} />
                )}
                {record && (
                    <BaseAdminForm
                        error={error}
                        ...
                        className={b('form')}
                        record={record}
                        fieldsComponent={BaseAdminFields}
                        onSave={onSave}
