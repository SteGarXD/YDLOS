...
export const BaseAdminForm = ({
    ...
    return (
        <div className={className}>
            ...
            {error && (
                <div className="alert alert-danger" role="alert" id="dl-admin-form-alert">
                    {error}
                </div>
            )}
            {loading && <div>{i18n('loading')}</div>}
            {React.createElement(fieldsComponent, {values, stores, onChange})}
            <div className={b('buttons-wrapper')}>
                <Button size={controlSize} onClick={() => onSave(values)} view="action">
                    {record?.id ? i18n('change') : i18n('create')}
                </Button>

                <Button size={controlSize} onClick={onCancel} view="flat">
                    {i18n('cancel')}
                </Button>
            </div>
        </div>
