...
import {Icon, Select, TextInput, SegmentedRadioGroup as RadioButton} from '@gravity-ui/uikit';
...
export const UserAdminFields = ({values, stores, onChange}) => {
        ...
        return (
            <>
                ...
                <TextInput
                    ...
                    hasClear
                />
                {values.c_claims?.indexOf('oidc') >= 0 ? null : (
                    <TextInput
                        ...
                        endContent={
                            <div
                                className={b('eye-icon-wrapper')}
                                onClick={() => setShowPassword(!showPassword)}
                            >
                                <Icon data={Eye} className={b('eye-icon')} />
                            </div>
                        }
    ...
    return (
        <>
            ...
            {values.id && (
                <RadioButton
                    value={String(values.b_disabled)}
                    onUpdate={(value) => onChange({target: {name: 'b_disabled', value: value}})}
                >
                    <RadioButton.Option value={'false'}>{i18n('active')}</RadioButton.Option>
                    <RadioButton.Option value={'true'}>{i18n('inactive')}</RadioButton.Option>
                </RadioButton>
            )}
        </>
    );
