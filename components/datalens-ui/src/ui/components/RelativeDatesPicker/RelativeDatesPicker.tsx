...
/** Ключ шкалы «месяцы» в state (см. SCALES в relative-datepicker) */
const SCALE_MONTH_KEY = 'M';
...
class RelativeDatesPicker extends React.Component<Props, State> {
    ...
    private getDatepickerHandler = (dateKey: keyof State): DatePickerProps['onUpdate'] => {
        return (value) => {
            ...
            const {start, end, scaleStart, scaleEnd} = this.state;
            const monthAbsoluteRange =
                Boolean(range) &&
                !withTime &&
                start === DATE_TYPES.ABSOLUTE &&
                end === DATE_TYPES.ABSOLUTE &&
                scaleStart === SCALE_MONTH_KEY &&
                scaleEnd === SCALE_MONTH_KEY;

            let iso: string | undefined;
            if (value && monthAbsoluteRange) {
                const dt = dateTimeUtc({input: value.toISOString()});
                if (dateKey === 'absoluteStart') {
                    iso = dt.startOf('month').toISOString();
                } else if (dateKey === 'absoluteEnd') {
                    iso = dt.endOf('month').toISOString();
                } else {
    ...
    private renderControl = (controlType: (typeof CONTROLS)[keyof typeof CONTROLS]) => {
        ...
        const monthAbsoluteCalendar =
            Boolean(range) &&
            !withTime &&
            startMode === DATE_TYPES.ABSOLUTE &&
            endMode === DATE_TYPES.ABSOLUTE &&
            scaleStart === SCALE_MONTH_KEY &&
            scaleEnd === SCALE_MONTH_KEY;
                    ...
                    <RadioGroup.Option value={DATE_TYPES.ABSOLUTE}>
                        ...
                        >
                            <DatePicker
                                ...
                                {...(monthAbsoluteCalendar
                                    ? {
                                          defaultMode: 'months' as const,
                                          modes: {
                                              days: false,
                                              months: true,
                                              quarters: false,
