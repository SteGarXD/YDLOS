...
import {ChartkitMenuDialogsQA, type StringParams, applyDashIntervalAliasSplitToStringParams} from 'shared';
...
export const ChartWidget = (props: ChartWidgetProps) => {
    ...
    const chartkitParams = React.useMemo(() => {
        ...
        return applyDashIntervalAliasSplitToStringParams(res);
    ...
    /*
     * treeState (+/- у плоского дерева рейсов) не должен менять requestId: иначе каждый клик — новый id,
     * отмена предыдущего /api/run и гонки с hash на дашборде. Остальные внутренние параметры — как раньше.
     */
    const hasChangedInnerParamsFromInside = React.useMemo(() => {
        if (!prevInnerParams || !innerParamsRef?.current) {
            return false;
        }
        return !isEqual(
            omit(innerParamsRef.current, 'treeState'),
            omit(prevInnerParams, 'treeState'),
        );
