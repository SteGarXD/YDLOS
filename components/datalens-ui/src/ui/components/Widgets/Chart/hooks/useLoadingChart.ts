...
import {DashTabItemControlSourceType, normalizeFlatTableTreeStateList, SHARED_URL_OPTIONS} from 'shared';
...
import {mergeTableTreeStateForDataProvider} from '../../../../libs/DatalensChartkit/ChartKit/plugins/Table/renderer/utils/flatTableRowTreeKeys';
import type {
...
export const useLoadingChart = (props: LoadingChartHookProps) => {
    ...
    const requestDataProps: DataProps = React.useMemo(() => {
        ...
        if (hasChangedOuterProps || hasChangedOuterParams) {
            ...
            /*
             * Дашборд: здесь `params` лежит поверх `filteredLocalParams`, иначе селекторы/связи перебивают локальное.
             * Но hash с `treeState` отстаёт на кадр от второго «+» у плоского дерева рейсов — внешний одно-ключевой
             * treeState затирает полный локальный массив, схлопывая раскрытия. Тот же merge, что в ChartKitBase.
             */
            const mergedTreeState = mergeTableTreeStateForDataProvider(
                params as StringParams,
                filteredLocalParams as StringParams,
            );
            if (mergedTreeState !== undefined) {
                newParams = {...newParams, treeState: mergedTreeState};
            }
            res = {
                ...res,
                params: newParams,
    ...
    }, [innerParamsRef, requestDataProps]);

    /**
     * Новая таблица рендерится через OpenSource ChartKit (не ChartKitBase): после /api/run в loadedData.params
     * часто нет treeState, хотя запрос ушёл с ним. Table по смене requestId синхронизирует baseline с пустым
     * treeState — схлопываются все «+», дальше клики ломаются. Для отрисовки накладываем клиентские changedParams.
     *
     * Дашборд: синхронизация outer→widget кладёт в changedParams даты/селекторы без ключа treeState (мы его
     * намеренно не шлём из loadChartData). Тогда только loadedData — в UI нет treeState, +/- не раскрывают.
     * Подмешиваем treeState из initialData (hash/merged props), если локальный changedParams его не задаёт.
     */
    const loadedDataForRender = React.useMemo(() => {
        if (!loadedData || loadedData.type !== 'table') {
            return loadedData;
        }
        const lp = loadedData.params || {};
        const ip = initialData.params || {};
        const cp = changedParams;
        /*
         * В defaults виджета на дашборде почти всегда есть treeState: [] (withRuntimeKeys).
         * Если подмешивать его при «cp без ключа treeState», merge с пустым ip даёт [] и затирает
         * раскрытие из lp/cр после клика по +. Берём initialData.treeState только если в hash реально
         * непустой набор ключей (шаринг/восстановление), а не дефолтный пустой массив.
         */
        const ipTreeKeys = normalizeFlatTableTreeStateList(
            ([] as string[]).concat(ip.treeState ?? []).filter(Boolean) as string[],
        );

        if (!cp || Object.keys(cp).length === 0) {
            if (ipTreeKeys.length === 0) {
                return loadedData;
            }
            const mergedTs = mergeTableTreeStateForDataProvider(
                ip as StringParams,
                lp as StringParams,
            );
            if (mergedTs === undefined) {
                return loadedData;
            }
            return {
                ...loadedData,
        ...
        const params: StringParams = {...lp, ...cp};
        if (!Object.prototype.hasOwnProperty.call(cp, 'treeState') && ipTreeKeys.length > 0) {
            const mergedTs = mergeTableTreeStateForDataProvider(
                ip as StringParams,
                params as StringParams,
            );
            if (mergedTs !== undefined) {
                params.treeState = mergedTs;
            }
        }
    ...
    }, [loadedData, changedParams, initialData]);
    ...
    const loadChartData = React.useCallback(async () => {
        ...
        const prevParams = prevInnerParamsRefCurrent as StringParams | undefined;
        const nextParams = requestDataProps.params;
        const onlyTreeStateChanged =
            prevParams !== undefined &&
            prevParams !== null &&
            !isEqual(prevParams, nextParams) &&
            isEqual(omit(prevParams, 'treeState'), omit(nextParams, 'treeState'));
        dispatch({
            type: WIDGET_CHART_SET_LOAD_SETTINGS,
            payload: {
                isSilentReload: Boolean(onlyTreeStateChanged),
                isReloadWithNoVeil: Boolean(onlyTreeStateChanged),
            },
        });
        ...
        try {
            ...
            if (hasChangedOuterParams) {
                /*
                 * Дашборд: не кладём treeState в UPDATE_DATA_PARAMS из этой синхронизации никогда.
                 * Снимок requestDataProps отстаёт от hash / второго «+» и затирает changedParams.treeState;
                 * раскрытия задаёт только handleChange. Итоговый запрос по-прежнему собирается в useMemo.
                 * SET_DATA_PARAMS при clearedOuterParams — полный payload (полная замена changedParams).
                 */
                const rawParams = requestDataProps.params;
                const useSetDataParams = Boolean(clearedOuterParams?.length);
                const payloadParams = useSetDataParams ? rawParams : omit(rawParams, 'treeState');
                dispatch({
                    type: useSetDataParams
                        ? WIDGET_CHART_SET_DATA_PARAMS
                        : WIDGET_CHART_UPDATE_DATA_PARAMS,
            ...
            // `getWidget` returns `null` on cancel and on HTTP 424 (cascading dash selectors / deps not ready).
            if (
                isCanceled ||
                loadedWidgetData === undefined ||
                ...
                // 424 is not a "hard" chart failure: stop loading without surfacing ChartKit error UI/noise.
                if (!isCanceled && loadedWidgetData === null) {
                    dispatch({type: WIDGET_CHART_SET_LOADING, payload: false});
                    dispatch({
                        type: WIDGET_CHART_SET_WIDGET_ERROR,
                        payload: {
                            isLoading: false,
                ...
                if (
                    ...
                    const options: ItemStateAndParamsChangeOptions | undefined =
                        needClearParamsQueue
                            ...
                            : undefined;
                    // Передаём в DashKit только фактически изменившиеся параметры виджета.
                    // Полный набор для hash собирается выше на уровне widget-хука по схеме текущей вкладки.
                    const paramsForDash = newParams as StringParams;
                    handleChangeCallback({
                        type: 'PARAMS_CHANGED',
                        data: {
                            params: paramsForDash,
                        },
                        options,
                    });
                }
    ...
    return {
        loadedData: loadedDataForRender,
        isLoading,
        isSilentReload,
        isReloadWithNoVeil,
        error,
