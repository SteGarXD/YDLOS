...
import type {DashSettings, StringParams} from 'shared';
...
import {CHART_WIDGET_DASH_RUNTIME_PARAM_KEYS} from 'ui/units/dash/utils/chartWidgetDashDefaults';
    ...
    const handleChangeCallback = React.useCallback(
        async (changedProps: OnChangeData) => {
            if (changedProps.type === 'PARAMS_CHANGED') {
                /*
                 * База — props.params из DashKit (живой hash), не currentTab.params из data.tabs.
                 * DashKit getAllowableChangedParams делает pick(..., Object.keys(tab.params)): лишние ключи
                 * отбрасываются и дают жёлтое «Параметры, которых нет в defaults» — из-за этого treeState
                 * мог теряться вместе с другими полями при рассинхроне.
                 */
                const incoming = (changedProps.data.params || {}) as StringParams;
                const dash = (dashMergedParams || {}) as StringParams;
                const allowedKeys = [
                    ...new Set([
                        ...Object.keys(currentTab.params || {}),
                        ...CHART_WIDGET_DASH_RUNTIME_PARAM_KEYS,
                    ]),
                ];
                const merged: StringParams = {
                    ...pick(dash, allowedKeys),
                    ...pick(incoming, allowedKeys),
                };
                const treeMerged = mergeTableTreeStateForDataProvider(dash, incoming);
                if (treeMerged !== undefined) {
                    merged.treeState = treeMerged;
                }
                onStateAndParamsChange(
                    {
                        params: merged,
        ...
        [currentTab.id, currentTab.params, dashMergedParams, onStateAndParamsChange],
    );
