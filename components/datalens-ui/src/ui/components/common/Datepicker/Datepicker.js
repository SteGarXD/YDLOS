...
export class Datepicker extends React.PureComponent {
    static propTypes = {
        ...
        enabledTabs: PropTypes.arrayOf(
            PropTypes.oneOf([TABS.DAY, TABS.WEEK, TABS.MONTH, TABS.QUARTER, TABS.YEAR]),
        ),
    ...
    static getDerivedStateFromProps(props, state) {
        ...
        if (props.enabledTabs?.length && !props.enabledTabs.includes(state.activeTab)) {
            changedState.activeTab = props.enabledTabs[0];
        }
    ...
    onRangeDateClick = ({start, end, isAllRangePicked, scrollCalendar}) => {
        ...
        // For fixed granularity selectors (scale=month) activeTab in state may stay "day".
        // Use effective tab that is actually rendered.
        const effectiveTab = this.props.scale || this.state.activeTab;
        const isMonthTab = effectiveTab === 'month';
        ...
        switch (true) {
            case pick === 1 || isAllRangePicked || !range: {
                ...
                // For month range selection we complete interval in one click.
                if (isAllRangePicked || !range || (range && isMonthTab && pick === 1)) {
                    pick = 0;
                }
    ...
    onControlPointerDown = () => {
        // Explicit user click: unlock focus so the picker can open.
    ...
    onInputFocus = () => {
        // While locked, immediately blur the input so no focus ring appears and picker stays closed.
        // The lock is set after onUpdate() and cleared only by an explicit pointer click.
    ...
    onClose = () => {
        ...
        const effectiveLastHash = this._lastCommittedHash ?? lastValidHash;

        if (effectiveLastHash === currentHash) {
            const searchText = getSearchText({
                from,
                to,
                format,
    ...
    onUpdate = () => {
        ...
        this._lastCommittedHash = getHashedData({from, to});
        // Lock focus: prevent any focus event (browser refocus, FloatingFocusManager, chart reload)
        // from reopening the picker. Unlocked only when the user explicitly clicks the control.
    ...
    renderContent(mobile) {
        const {scale, range, showApply, enabledTabs} = this.props;
