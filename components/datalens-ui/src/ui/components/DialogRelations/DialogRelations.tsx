...
const DialogRelations = (props: DialogRelationsProps) => {
    ...
    const [selectedRelationSubjectId, setSelectedRelationSubjectId] = React.useState<string>(
        currentWidget?.type === DashTabItemType.GroupControl
            ? currentWidget.data.group[0].id
            : currentWidget?.id || '',
    );
    ...
    const currentWidgetId = selectedRelationSubjectId || itemId || currentWidgetMeta?.widgetId || '';
    const [changedWidgets, setChangedWidgets] = React.useState<WidgetsTypes>({});
    const prevPreparedWidgetIdRef = React.useRef<string>('');
    const pendingPreparedSyncIdRef = React.useRef<string>('');
    ...
    const handleItemChange = (value: string[]) => {
        ...
        setSelectedRelationSubjectId(selectedId);
        pendingPreparedSyncIdRef.current = selectedId;
        ...
        setItemId(newWidgetData?.isItem ? selectedId : null);
    ...
    React.useEffect(() => {
        // When switching key widget/selector in the top Select, relations must be swapped immediately.
        // Otherwise UI can keep previous widget relations until dialog reopen.
        if (currentWidgetId !== prevPreparedWidgetIdRef.current) {
            prevPreparedWidgetIdRef.current = currentWidgetId;
            setPreparedRelations([]);
            return;
        }
        if (pendingPreparedSyncIdRef.current && pendingPreparedSyncIdRef.current === currentWidgetId) {
            setPreparedRelations(relations);
            pendingPreparedSyncIdRef.current = '';
            return;
        }
        if (!preparedRelations?.length && relations.length) {
            setPreparedRelations(relations);
        }
    }, [currentWidgetId, relations, preparedRelations]);
        ...
        >
            ...
            <Dialog.Body className={b('container')}>
                <Select
                    ...
                    /* Пустой массив вместо undefined — иначе uikit Select (useControlledState) ругается при появлении id после загрузки меты */
                    value={currentWidgetId ? [currentWidgetId] : []}
