...
import {
    ...
    flattenPerControlStringParams,
    getPerControlParamUpdatesWhenDateChanges,
    isFlatControlParamMap,
} from './utils';
...
class GroupControl extends React.PureComponent<PluginGroupControlProps, PluginGroupControlState> {
    ...
    componentDidMount() {
        const navEntry = (window.performance.getEntriesByType?.('navigation')?.[0] || null) as
            | PerformanceNavigationTiming
            | null;
        const isPageReload = navEntry?.type === 'reload';

        if (isPageReload && this.propsControlData.group?.length) {
            const defaultParams =
                this.propsControlData.group.reduce(
            ...
            const resetStateParams = this.getUpdatedGroupParams({
                params: defaultParams,
                meta: {version: LOCAL_META_VERSION, queue: []},
            });
            this.initialParams = resetStateParams;
            this.setState({stateParams: resetStateParams}, () => this.init());
            return;
        }
    ...
    componentDidUpdate(prevProps: Readonly<PluginGroupControlProps>) {
        ...
        if (hasParamsChanged || hasDataChanged) {
            ...
            this.props.data.group?.forEach((groupItem) => {
                const paramKey = Object.keys(groupItem.defaults || {})[0];
                ...
                if (isEqual(initialParams, newPropsParams)) {
                    ...
                    const nextParams = {...this.props.params[groupItem.id]};
                    const currentParams = this.state.stateParams[groupItem.id] || {};
                    const titleText = String(groupItem.title || '').toLowerCase();
                    const isFlightOrDirectionSelector =
                        titleText.includes('рейс') ||
                        titleText.includes('flight') ||
                        titleText.includes('напр') ||
                        titleText.includes('direction');
                    if (paramKey && isFlightOrDirectionSelector) {
                        const nextValue = nextParams[paramKey];
                        ...
                        const currentIsNonEmpty = !(
                            currentValue === undefined ||
                            currentValue === null ||
                            currentValue === '' ||
                            (Array.isArray(currentValue) && currentValue.length === 0)
                        );
                        // Prevent Apply-cycle from wiping already selected flight/direction values.
                        if (nextIsEmpty && currentIsNonEmpty) {
                            nextParams[paramKey] = currentValue as any;
                        }
                    }
                    updatedStateParams[groupItem.id] = nextParams;
    ...
    private onChange = ({
        ...
        if (!controlData.buttonApply || callChangeByClick) {
            let paramsToApply: StringParams | Record<string, StringParams> = params;
            let groupItemIds = this.getControlsIds({data: controlData, controlId});
            if (controlId && isFlatControlParamMap(params) && controlData.group?.length) {
                const perControl = getPerControlParamUpdatesWhenDateChanges({
                    group: controlData.group,
                    changedControlId: controlId,
                    incomingDelta: params,
                    currentGroupStateParams: this.state.stateParams,
                });
                paramsToApply = flattenPerControlStringParams(perControl);
                groupItemIds = [...new Set([...groupItemIds, ...Object.keys(perControl)])];
            }
            this.props.onStateAndParamsChange({params: paramsToApply}, {groupItemIds});
            this.localMeta.queue = [];
            return;
        }
        ...
        if (controlId) {
            ...
            const perControlUpdates =
                isFlatControlParamMap(params) && controlData.group?.length
                    ? getPerControlParamUpdatesWhenDateChanges({
                          group: controlData.group,
                          changedControlId: controlId,
                          incomingDelta: params,
                    ...
                    : {[controlId]: params as StringParams};
            if (controlData.updateControlsOnChange && controlData.buttonApply) {
                this.setState({
                    stateParams: this.mergeMultiControlParamDeltas(perControlUpdates),
    ...
    private mergeMultiControlParamDeltas = (updates: Record<string, StringParams>) => {
        const newParams: Record<string, StringParams> = {...this.state.stateParams};
        for (const [cid, delta] of Object.entries(updates)) {
            newParams[cid] = {...(newParams[cid] || {}), ...delta} as StringParams;
        }
    ...
    private renderControl(item: DashTabItemControlSingle) {
        ...
        return (
            <Control
                ...
                id={item.id}
                data={item}
                group={this.propsControlData.group || []}
                groupStateParams={this.state.stateParams}
