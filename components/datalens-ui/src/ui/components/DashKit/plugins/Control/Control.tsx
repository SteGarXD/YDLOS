...
import {
    appendDatepickerParamsFromScheme,
...
export interface PluginControlProps extends PluginWidgetProps, ContextProps, ControlSettings {
    ...
    /** Синхронизация списков селекторов после редактора групп (см. bumpFlightGroupsDatasetRefresh). */
...
function isBenignControlDependencyError(error: any) {
    const status = Number(error?.response?.status ?? error?.status);
    return status === 424 || status === 427;
}
...
class Control extends React.PureComponent<PluginControlProps, PluginControlState> {
    ...
    /** Повтор init при кратковременном ECONNRESET / 502–504 без шума в консоли. */
    private _transientInitRetries = 0;
    /** После validateStatus для 424 axios не reject — повтор init, пока каскад селекторов не готов. */
    private _dependency424Retries = 0;
    ...
    componentDidUpdate(prevProps: Readonly<PluginControlProps>) {
        ...
        if (prevNonce !== nextNonce && this.state.isInit) {
            // Принудительный re-init: bypass skipReload, иначе content селекта может не обновиться
            // и в UI остаётся старое название группы после rename.
            this.setState({
                status: LOAD_STATUS.PENDING,
                silentLoading: true,
            });
    ...
    filterSignificantParams(params: StringParams) {
        ...
        if (loadedData && loadedData.usedParams && dependentSelectors) {
            const picked = pickSignificantParamsSubset(
                params,
                loadedData.usedParams,
                this.props.defaults as StringParams | undefined,
            );
        ...
        const defaultKeys = Object.keys(this.props.defaults || {});
        if (defaultKeys.length > 0) {
            return applyDashIntervalAliasSplitToStringParams(pick(params, defaultKeys));
        }
    ...
    setLoadedData = (loadedData: ResponseSuccessControls, status: LoadStatus) => {
        ...
        if (status === LOAD_STATUS.SUCCESS) {
            this._transientInitRetries = 0;
            this._dependency424Retries = 0;
        }
    ...
    init = async () => {
        try {
            ...
            const normalizedParams: StringParams = {};
            for (const [key, val] of Object.entries(this.actualParams)) {
                const raw = Array.isArray(val) ? String(val[0] ?? '') : String(val ?? '');
                if (raw.startsWith('__interval_')) {
                    const m = raw.match(/^__interval_(.+?)_(.+)$/);
                    normalizedParams[key] = m ? m[1].replace('T', ' ').split('.')[0] : raw;
                } else {
            ...
            if (response.status === 424 || response.status === 427) {
                // Иначе isInit остаётся false → reload() навсегда отключён, селекторы «висят».
                if (this.state.isInit === false) {
                    this.setState({isInit: true});
                }
                const maxSoft = 24;
                if (this._dependency424Retries < maxSoft && !this._isUnmounted) {
                    this._dependency424Retries += 1;
                    const delayMs = Math.min(40 + this._dependency424Retries * 70, 1200);
                    window.setTimeout(() => {
                        if (!this._isUnmounted && !this.context?.skipReload) {
                            void this.init();
                        }
                    }, delayMs);
                }
            ...
            if (isBenignControlDependencyError(error)) {
                return;
            }
            if (
                isTransientControlInfrastructureError(error) &&
                this._transientInitRetries < 3 &&
                !this._isUnmounted
            ) {
                this._transientInitRetries += 1;
                const delayMs = 250 + this._transientInitRetries * 250;
                window.setTimeout(() => {
                    if (!this._isUnmounted) {
                        this.reload({silentLoading: true});
                    }
                }, delayMs);
                return;
            }
