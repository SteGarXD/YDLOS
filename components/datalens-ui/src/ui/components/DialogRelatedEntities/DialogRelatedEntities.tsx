...
export const DialogRelatedEntities = ({onClose, visible, entry}: DialogRelatedEntitiesProps) => {
    ...
    for (const updatedEntry in updatedEntities) {
        if (
            Object.prototype.hasOwnProperty.call(updatedEntities, updatedEntry) &&
            updatedEntities[updatedEntry]
        ) {
    ...
    useEffect(() => {
        ...
        Object.values(relations).forEach((group) => {
            group.forEach((rel) => {
                if (updatedEntities[rel.entryId] === undefined) {
                    _updatedEntities[rel.entryId] = false;
                }
            });
        });
        ...
        Utils.getRoles({})
            .then((roles) => {
                ...
                roleList.forEach((roleItem: any) => {
                    if (
                        fullAccesses.findIndex((item: any) => {
                            return roleItem.role_id === item.role_id;
                        }) < 0
                    ) {
                        fullAccesses.push({
                ...
                const arr: any[] = [];
                Object.keys(updatedEntities).forEach((entityKey) => {
                    if (!updatedEntities[entityKey]) {
                        return;
                    }
                    fullAccesses.forEach((accessItem: any) => {
                        arr.push({id: entityKey, ...accessItem});
                    });
                });
                Utils.setAccesses([arr.map((item) => ({...item, destroy: true}))])
                    .then(() => {
                        setIsLoading(false);
                        onClose({status: EntryDialogResolveStatus.Close});
                    })
                    .catch(() => {
                        setIsLoading(false);
                        console.error(
                            'Error updating accesses of entities',
                            updatedEntities,
                            fullAccesses,
                        );
                    });
    ...
    return (
        <Dialog onClose={handleClose} open={visible} className={b()} disableHeightTransition={true}>
            ...
            <Dialog.Body className={b('body')}>
                ...
                <Card className={b('warning-card')} theme="warning" size="l">
                    {i18n('selected-entities-assign-description')}
                </Card>
            </Dialog.Body>
            <Dialog.Footer>
                <Button
                ...
                <Button size="l" view="action" disabled={isLoading} onClick={handleApply}>
                    {i18n('apply')}
                </Button>
            </Dialog.Footer>
        </Dialog>
