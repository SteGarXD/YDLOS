...
const prepareValues = (chartData: any) => {
    ...
    const cellStyles: (string | null)[][] = [];
    allTableRows.forEach((row) => {
        const cells = row.values?.map((value: any) => ({value})) || row.cells;
        cellStyles.push(
            cells.map((cell: any) => {
                const bg = cell.css?.backgroundColor;
                return typeof bg === 'string' && bg && !bg.startsWith('var(') ? bg : null;
            }),
        );
    });
...
export async function exportEntries(req: Request, res: Response) {
    ...
    const r: any = req;
    const host = r.body['host'] || 'http://localhost:8080';
    ...
    if (r.body['links']) {
        ...
        const chartData = await getChartData(
            host,
            req.headers['x-rpc-authorization'] as string,
            links,
            r.body['params'],
        );
        ...
        const publicOutputPath = path.join(
            exportPath,
            `${exportFilename + '-' + Date.now()}.${r.body['outputFormat']}`,
        );

        const metadata: Array<{csv_data_name: string} & Record<string, string>> = [];
        ...
        for (let i = 0; i < filteredLinks.length; i++) {
            if (chartData[filteredLinks[i]].extra.datasets) {
                const sheetName =
                    (chartData[filteredLinks[i]].key.split('/').length > 1
                        ? chartData[filteredLinks[i]].key.split('/')[1]
                        : filteredLinks[i]) +
                    '-' +
                    Date.now();

                const publicOutputCSVPath = path.join(
                    exportPath,
                    `${sheetName.replace(clearRegexp, '_')}.${r.body['formSettings'].format}`,
                );
                ...
                ['__template_name', '__sheet_name', '__mapping'].forEach((name) => {
                    if (chart.defaultParams[name]?.length > 0) {
                        chartProps[String(name).replace('__', '')] = String(
                            chart.defaultParams[name][0],
                        );
                    }
                });
                ...
                const response = await stringifyData(
                    host,
                    chartData[filteredLinks[i]],
                    req.headers['x-rpc-authorization'] as string,
                    r.body,
                );
        ...
        const resSpawn = child_process.spawnSync(ctx.config.python || 'python3', [
            pythonScript,
            `OUTPUT_NAME="${publicOutputPath}"`,
            `SEP=${SEPARATOR}`,
            `META_NAME="${metaPath}"`,
        ]);
