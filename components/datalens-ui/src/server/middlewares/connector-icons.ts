...
export function getConnectorIconsMiddleware({
    ...
    return async function connectorIconsMiddleware(
        ...
        // YDL OS: не вызывать gateway для статики (css, js, ico и т.д.) — иначе /wizard/fonts.css таймаутит и отдаётся HTML
        if (
            req.method === 'GET' &&
            /\.(css|js|ico|woff2?|png|svg|jpg|jpeg|gif)(\?|$)/i.test(req.path)
        ) {
            res.locals.connectorIcons = [];
            return next();
        }
