...
export default {
    ...
    usMasterToken: process.env.US_MASTER_TOKEN || 'us-master-token',
    ...
    chartkitSettings: {
        // YDL-OS: не подключаем внешние скрипты Highcharts (CDN). Политика и остаточные риски — в datalens/datalens/CHARTS_AND_LICENSES.md.
        highcharts: {
            enabled: false,
        },
        yandexMap: {
    ...
    // OIDC / Keycloak: дефолты из Meridian-Demo (http://localhost:8090, realm meridian-demo, client datalens-client)
    oidc: isTrueArg(process.env.OIDC) || process.env.APP_ENV === 'development',
    oidc_issuer: process.env.OIDC_ISSUER || 'http://localhost:8090/realms/meridian-demo',
    oidc_base_url: process.env.OIDC_BASE_URL || 'http://localhost:8090/realms/meridian-demo',
    oidc_client_id: process.env.OIDC_CLIENT_ID || 'datalens-client',
    oidc_secret: process.env.OIDC_SECRET || '',
    oidc_name: process.env.OIDC_NAME || 'Keycloak',
    ...
    authSignupDisabled:
        isTrueArg(process.env.AUTH_SIGNUP_DISABLED) ||
        process.env.APP_INSTALLATION === AppInstallation.Opensource,
