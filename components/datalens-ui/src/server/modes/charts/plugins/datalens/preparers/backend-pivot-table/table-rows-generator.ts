...
import {TABLE_EMPTY_MEASURE_CELL_BG, TABLE_TOTALS_STYLES} from '../../../constants/misc';
...
function getRowCellMetadata(args: GetRowCellMetadataArgs): ChartkitCell {
    ...
    if (value === null) {
        // YDL OS: пустые ячейки ЗПК % и Млн. р — светло-серый только в теле; в ИТОГО — серый из TABLE_TOTALS_STYLES
        if (!isTotalsRowValue) {
            const title = field?.fakeTitle || (field as any)?.title;
            if (title && ['ЗПК %', 'Млн. р'].includes(String(title))) {
                cell.css = {...(cell.css || {}), backgroundColor: TABLE_EMPTY_MEASURE_CELL_BG};
            }
        }
        return cell;
    }
...
export const generateTableRows = ({
    ...
    const rows: any[] = pivotData.rows.reduce(
        (acc: any[], pivotDataRow: PivotDataRows, rowIndex) => {
            ...
            const lastHeader = pivotDataRow.header?.length
                ? pivotDataRow.header[pivotDataRow.header.length - 1]
                : null;
            const rowMeasureTitle =
                lastHeader &&
                getDatasetFieldFromPivotTableValue(
                    lastHeader as PivotDataCellValue[],
                    fieldsItemIdMap,
                    fieldDict,
                )?.fakeTitle;
            ...
            pivotDataRow.values.forEach((pivotDataRowValue: PivotDataRowsValue, valueIndex) => {
                if (!pivotDataRowValue) {
                    ...
                    const isLastDataColumn =
                        pivotDataRow.values && valueIndex === pivotDataRow.values.length - 1;
                    if (
                        !isTotalCell &&
                        !isLastDataColumn &&
                        rowMeasureTitle &&
                        ...
                        emptyCell.css = {
                            ...(emptyCell.css || {}),
                            backgroundColor: TABLE_EMPTY_MEASURE_CELL_BG,
                        };
                    }
                    row.cells.push(emptyCell);
