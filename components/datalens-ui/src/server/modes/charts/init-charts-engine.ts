...
export function initChartsEngine({
    ...
    // В dev/full режиме конфиг может не иметь getSourcesByEnv (общий common-конфиг без charts-части).
    // В таком случае не инициализируем sources, чтобы не падать при старте.
    if (
        typeof (config as AppConfig & {getSourcesByEnv?: (env: AppEnvironment) => unknown})
            .getSourcesByEnv === 'function'
    ) {
        config.sources = (
            config as AppConfig & {getSourcesByEnv: (env: AppEnvironment) => unknown}
        ).getSourcesByEnv(typedAppEnv);
    }

    const chartsConfig = (
        config as AppConfig & {chartsEngineConfig?: AppConfig['chartsEngineConfig']}
    ).chartsEngineConfig || {
        secrets: {},
        flags: {},
    ...
    // Проставляем дефолт обратно в config, чтобы остальные места (data-fetcher и т.п.) не падали.
    (config as AppConfig & {chartsEngineConfig: typeof chartsConfig}).chartsEngineConfig =
        chartsConfig;

    if (endpoints?.api?.us && chartsConfig.usEndpointPostfix !== undefined) {
        config.usEndpoint = endpoints.api.us + chartsConfig.usEndpointPostfix;
    }
    ...
    return new ChartsEngine({
        config,
        secrets: chartsConfig.secrets ?? {},
        flags: chartsConfig.flags ?? {},
        plugins,
        telemetryCallbacks: chartsConfig.enableTelemetry ? telemetryCallbacks : undefined,
        cacheClient,
        beforeAuth,
        afterAuth,
