...
export function getFeaturesConfig(appEnvironment?: string) {
    ...
    // prod → production (как в endpoints; в features-list только development | production)
    const envKey = appEnvironment === 'prod' ? 'production' : appEnvironment;

    FEATURES_LIST.forEach(({name, state}) => {
        // @ts-ignore
        registerFeature(name, state[envKey]);
    });
