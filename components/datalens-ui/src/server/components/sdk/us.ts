...
class US {
    ...
    static async universalService(
        ...
        try {
            const axios = getAxios(ctx.config);
            const {data: result} = await axios({
                ...
                headers: {
                    ...
                    /** Не пробрасываем весь req.headers — только master по тем же правилам, что gateway. */
                    ...(shouldAttachUsMasterToken(ctx)
    ...
    static async oidcAuth(data: any, ctx: AppContext): Promise<any> {
        try {
            const axios = getAxios(ctx.config);
            const {data: result} = await axios({
                method: 'GET',
    ...
    static async createEntry(
        ...
        try {
            const {data: result} = await getAxios(ctx.config)({
                ...
                headers: mergeUsMasterTokenHeader(headers, ctx),
                data,
            });
