...
export function csvConverter(
    ...
    if (formSettings.format === 'ods') {
        ...
        try {
            fs.writeFileSync(publicOutputCsvPath, csvContent.toString());
        } catch (err) {
            ctx.logError(`EXPORT_ODS_DATA_WRITE_ERROR`, {
                outputPath: publicOutputCsvPath,
                message: `Ошибка сохранения файла CSV: ${String(err)}`,
            });
            req.ctx.stats('exportSizeStats', {
                datetime: Date.now(),
                exportType: 'ods',
        ...
        const resSpawn = child_process.spawnSync(context.config.python || 'python3', [
            pythonScript,
            `FILE_PATH="${publicOutputCsvPath}"`,
        ]);
        if (resSpawn.stderr.byteLength > 0) {
            ctx.logError(`EXPORT_ODS_DATA_WRITE_ERROR`, {
                outputPath: publicOutputCsvPath,
