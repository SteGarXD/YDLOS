...
import {
    getUsMasterTokenValue,
    shouldAttachUsMasterToken,
} from '../../../../../utils/us-master-token';
...
function injectMetadata(headers: IncomingHttpHeaders, ctx: AppContext): IncomingHttpHeaders {
    ...
    const merged: IncomingHttpHeaders = {...headers, ...metadata};

    /**
     * Opensource / dev: POST /v1/entries при сохранении чарта без master → US.AUTH.UNAUTHORIZED_ACCESS (401).
     * Те же условия и значение, что в gateway.ts / mergeUsMasterTokenHeader (токен с сервера, не из браузера).
     */
    if (shouldAttachUsMasterToken(ctx)) {
        merged[US_MASTER_TOKEN_HEADER] = getUsMasterTokenValue(ctx);
    }
