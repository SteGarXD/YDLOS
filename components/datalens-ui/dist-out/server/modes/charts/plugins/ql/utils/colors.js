...
exports.compareTextWithNumber = compareTextWithNumber;
exports.getGradientColor = getGradientColor;
exports.getColorAlexPetrov = getColorAlexPetrov;
exports.getColorsForNames = getColorsForNames;
exports.getColorFieldsFromDistincts = getColorFieldsFromDistincts;
...
(function (ColorSchemeType) {
    ColorSchemeType["Gradient"] = "gradient";
    ColorSchemeType["Default"] = "default";
...
var LineColorType;
(function (LineColorType) {
    LineColorType["Auto"] = "auto";
    LineColorType["Gradient"] = "gradient";
    LineColorType["Fixes"] = "fixed";
...
function putIfAbsent(map, key, value) {
    if (!lodash_1.default.has(map, key)) {
        map[key] = value;
    }
}
...
function isUniqueGradientedLabels(labels) {
    ...
    for (let i = 1; i < labels.length; ++i) {
        ...
        if (!lodash_1.default.isEqual(firstLabelPrefixOrSuffix, currentLabelPrefixAndSuffix)) {
            return false;
        }
    }
    return true;
...
function constructColorMap(colorIndexes, labels) {
    const colorMap = {};
    if (lodash_1.default.size(colorIndexes) > 0) {
        const entryList = lodash_1.default.entries(colorIndexes);
...
function getInputSetGradientColorMap(colorLabels, uniqueValues) {
    ...
    for (let i = 0; i < uniqueValues.length; ++i) {
        ...
        const filteredEntries = lodash_1.default.entries(colorLabels).filter((e) => e[1] === value);
...
function getMapOrder(colorNumbers) {
    if (lodash_1.default.size(colorNumbers) > 1) {
        const initialKeys = GradientColorValues.filter((color) => lodash_1.default.has(colorNumbers, color.ordinal));
...
function addMissingPointsToGradientMap(colorNumbers, setSize) {
    ...
    const keys = GradientColorValues.filter((color) => lodash_1.default.has(colorNumbers, color.ordinal));
    ...
    while (i < allKeys.length && j < keys.length) {
        ...
        if (order > 0) {
            if (j > 0) {
                ...
                const absentKeyNumber = Math.trunc(((keyNumber - pKeyNumber) * (absentKeyOrdinal - pKeyOrdinal)) /
...
function getUniqueGradientedLabels(labels, colorLabels) {
    const strings = [];
    strings.push(...labels);
    strings.push(...lodash_1.default.values(colorLabels).filter((s) => s));
    strings.sort(compareTextWithNumber);
    return lodash_1.default.uniqWith(strings, (a, b) => a === b);
}
...
function getColorOrAutoColor(colorScheme, label) {
    if (lodash_1.default.has(colorScheme, label)) {
...
function getColorsForNames(names, colorSchemeParams) {
    ...
    const namesForColors = uniqNames.size <= 1 ? names.map((_, index) => `uniqName${index}`) : names;
...
function getColorFieldsFromDistincts(distinctsMap, fields, usedAxisFields) {
    let colors;
    let colorIndex = fields.findIndex((column) => column.title === constants_1.QUERY_ALIAS_TITLE);
    if (colorIndex === -1) {
        colorIndex = fields.findIndex((column) => column.title === constants_1.QUERY_TITLE);
    }
    ...
    const generateGuidToFieldMap = (acc, f) => Object.assign(acc, { [f.guid]: f });
    const fieldByGuidMap = fields.reduce(generateGuidToFieldMap, {});
    const usedFieldsByGuidMap = usedFields.reduce(generateGuidToFieldMap, {});
