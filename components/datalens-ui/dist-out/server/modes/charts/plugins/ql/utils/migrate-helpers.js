...
exports.migrateTableVisualization = exports.migrateMetricVisualization = exports.migratePieVisualization = exports.migrateLineVisualization = void 0;
const shared_1 = require("../../../../../../shared");
const misc_helpers_1 = require("./misc-helpers");
const findIndexes = ({ fields, forbiddenIndexes }) => fields.reduce((result, field, fieldIndex) => {
    if ((field.cast === shared_1.DATASET_FIELD_TYPES.INTEGER ||
        field.cast === shared_1.DATASET_FIELD_TYPES.FLOAT) &&
...
const migrateLineVisualization = ({ order, fields, rows, }) => {
    ...
    if (order.length) {
        ...
        if (xDeclared && xIndex === -1) {
            xIndex = fields.findIndex((_column, index) => !yIndexes.includes(index) && !colorIndexes.includes(index));
        }
        if (yDeclared) {
        ...
        colorIndexes = fields
            ...
            const homogeneousValues = new Set();
...
exports.migrateLineVisualization = migrateLineVisualization;
const migratePieVisualization = ({ order, fields, }) => {
    let colorIndex = -1;
    let measureIndex = -1;
    let collectingColor = false;
...
exports.migratePieVisualization = migratePieVisualization;
const migrateMetricVisualization = ({ order, fields, }) => {
    let collectingMeasure = false;
    let measureIndex = -1;
    order.forEach((item) => {
        const itemIsGroup = (0, misc_helpers_1.isGroup)(item);
...
exports.migrateMetricVisualization = migrateMetricVisualization;
const migrateTableVisualization = ({ order, fields, }) => {
...
exports.migrateTableVisualization = migrateTableVisualization;
