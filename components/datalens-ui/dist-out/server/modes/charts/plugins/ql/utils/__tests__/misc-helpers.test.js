...
const commonBuildSourceArgsSet = {
    ...
    qlConnectionTypeMap: { postgres: shared_1.ConnectorType.Postgres },
    features: {},
};
...
const expectedBuildSourceResultSet = {
    ...
    method: 'post',
    data: {
        sql_query: "select built_year, iznos from public.sample where built_year in (E'1995') limit 10",
        params: {},
    },
};
...
const expectedBuildSourceResultSingle = {
    ...
    method: 'post',
    data: {
        sql_query: "select built_year, iznos from public.sample where built_year = E'1995' limit 10",
        params: {},
    },
};
...
jest.mock('../../../../../../registry', () => {
    return {
        registry: {
            getConvertConnectorTypeToQLConnectionType: () => connection_1.convertConnectionType,
        },
    };
});
describe('buildSource', () => {
    it('should work correctly with parameter-set substitution', () => {
        const buildSourceResult = (0, misc_helpers_1.buildSource)(mockedBuildSourceArgsSet);
    ...
    it('should work correctly with substitution of parameter "scalar" value', () => {
        const buildSourceResult = (0, misc_helpers_1.buildSource)(mockedBuildSourceArgsSingle);
    ...
    it('should work correctly with parameter-set substitution where value is already wrapped by user', () => {
        const buildSourceResult = (0, misc_helpers_1.buildSource)(mockedBuildSourceArgsPrewrapped);
...
describe('iterateThroughVisibleQueries', () => {
    it('should execute callback only for queries where hidden property is falsy', () => {
        ...
        (0, misc_helpers_1.iterateThroughVisibleQueries)(queries, cb);
        expect(cb.mock.calls.length).toEqual(4);
        expect(cb.mock.calls.every((args) => {
            const index = args[1];
            return index !== 1;
        }));
    });
...
describe('doesQueryContainOrderBy', () => {
    it('should return true when order by is used normally', () => {
        const queryContainsOrderBy = (0, misc_helpers_1.doesQueryContainOrderBy)('select a from b order by a desc');
        expect(queryContainsOrderBy).toEqual(true);
    });
    it('should return false when order by is not used', () => {
        const queryContainsOrderBy = (0, misc_helpers_1.doesQueryContainOrderBy)('select a from b');
    ...
    it('should return true when order by is used normally, but query contains -- in quotes', () => {
        const queryContainsOrderBy = (0, misc_helpers_1.doesQueryContainOrderBy)(`select a from b where a like '%--%' order by a`);
        expect(queryContainsOrderBy).toEqual(true);
    });
    it('should return false when order by is used as a string', () => {
        const queryContainsOrderBy = (0, misc_helpers_1.doesQueryContainOrderBy)(`select a from b where a = 'order by'`);
        expect(queryContainsOrderBy).toEqual(false);
    });
});
