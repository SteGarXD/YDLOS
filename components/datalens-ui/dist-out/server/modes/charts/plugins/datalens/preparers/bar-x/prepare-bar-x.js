...
exports.prepareBarX = prepareBarX;
...
function prepareBarX(args) {
    var _a, _b, _c;
    const { placeholders, resultData, colors, colorsConfig, sort, labels, idToTitle, idToDataType, visualizationId, datasets = [], shared, shapesConfig, segments, layerChartMeta, usedColors, ChartEditor, disableDefaultSorting = false, defaultColorPaletteId, } = args;
    ...
    const x2IsNumber = Boolean(x2DataType && (0, misc_helpers_1.isNumericalDataType)(x2DataType));
    const x2IsDate = Boolean(x2DataType && (0, shared_1.isDateField)({ data_type: x2DataType }));
    const x2IsPseudo = Boolean(x2 && x2.type === constants_1.PSEUDO);
    const isHtmlX = (0, shared_1.isHtmlField)(x);
    ...
    const gradientMode = colorItem && (0, misc_helpers_1.isGradientMode)({ colorField: colorItem, colorFieldDataType, colorsConfig });
    ...
    if (mergedYSections.length) {
        ...
        orderedLineKeys.forEach((lineKeys, lineKeysIndex) => {
            lineKeys.forEach((lineKey) => {
                ...
                const graph = {
                    ...
                    data: categories
                        ...
                        if (line.segmentNameKey) {
                            ...
                            let tooltipPointName = `${currentSegment.title}: ${line.title}`;
                            if (isHtmlSegment) {
                                tooltipPointName = (0, ui_sandbox_1.wrapHtml)(tooltipPointName);
                            }
                            point.custom = {
                                tooltipPointName,
                            };
                        ...
                        if (!isXCategoryAxis) {
                            ...
                            point.x = pointX;
                            if (x && (0, misc_helpers_1.isNumericalDataType)(x.data_type)) {
                ...
                if (isActionParamsEnable) {
                    ...
                    if ((0, shared_1.isDimensionField)(colorItem)) {
        ...
        if (x && isXCategoryAxis) {
            const categoriesFormatter = (0, misc_helpers_1.getCategoryFormatter)({
                field: { ...x, data_type: xDataType !== null && xDataType !== void 0 ? xDataType : x.data_type },
            });
            return {
                graphs,
