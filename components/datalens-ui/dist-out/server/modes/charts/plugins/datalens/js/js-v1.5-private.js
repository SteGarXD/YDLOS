...
exports.fallbackJSFunctionPrivate = void 0;
...
const prepare_single_result_1 = __importDefault(require("./helpers/misc/prepare-single-result"));
    ...
    Object.keys(data).forEach((key) => {
        ...
        if (mergedRows.length === 0) {
            ...
            if (data[key].result.data.Data.length) {
                let targetRow;
                // Merge all values
                data[key].result.data.Data.forEach((row, i) => {
                ...
                // Merge all types
                data[key].result.data.Type[1][1].forEach((type) => {
                    mergedOrder.push({
            ...
            const fieldsInMaster = mergedOrder.map((entry) => Array.isArray(entry) ? entry[0].title : entry.title);
            ...
            neededLinks.forEach((link, linkIndex) => {
                ...
                const indexInCurrent = fieldsInCurrent.findIndex((title) => title === currentFieldData.title);
            ...
            const newMergedRows = [];
            // Merge values
            const rows = data[key].result.data.Data;
            rows.forEach((row) => {
                let possibleTargetRows;
                ...
                neededLinks.forEach((_link, linkIndex) => {
                    ...
                    possibleTargetRows = (possibleTargetRows || mergedRows).filter((someRow) => {
...
const fallbackJSFunctionPrivate = ({ options, features, palettes, defaultColorPaletteId, }) => {
    let data;
    let shared;
    let ChartEditor;
    if ('shared' in options[0]) {
        ...
        ChartEditor = options[0].ChartEditor;
    }
    else {
        data = options[0];
        shared = options[1];
    ...
    shared = (0, config_helpers_1.mapChartsConfigToServerConfig)(shared);
    ...
    if (layers) {
        ...
        layers.forEach((layer) => {
            ...
            datasetsMeta = datasetsIds.map((id, datasetIndex) => {
                ...
                const fieldsList = layerMergedData.fieldsByDataset[id].map((item) => {
        ...
        datasetsMeta = datasetsIds.map((id, datasetIndex) => {
            ...
            const fieldsList = mergedData[0].fieldsByDataset[id].map((item) => {
                const fieldId = item.guid || item.id;
                return {
                    title: idToTitle[fieldId],
                    guid: fieldId,
    ...
    if (layers) {
        ...
        layers.forEach((layer, layerIndex) => {
            ...
            const localResult = (0, prepare_single_result_1.default)({
                resultData,
                shared: shared,
                visualization: layer,
                idToTitle,
        ...
        result = (0, prepare_single_result_1.default)({
            resultData,
            shared: shared,
...
exports.fallbackJSFunctionPrivate = fallbackJSFunctionPrivate;
