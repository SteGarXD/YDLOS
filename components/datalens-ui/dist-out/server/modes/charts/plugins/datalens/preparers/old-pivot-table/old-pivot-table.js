...
/** 1.2.1 Двухуровневые заголовки: день недели (пн, вт, …) для колонок с датой */
function getDayOfWeekShort(value) {
    var _a;
    if (value == null)
        return '';
    const m = (0, moment_1.default)(value);
    return m.isValid() ? (_a = DAY_OF_WEEK_SHORT[m.day()]) !== null && _a !== void 0 ? _a : '' : '';
}
function sorter(a, b) {
    if (a === null) {
        return 1;
    ...
    return misc_helpers_1.collator.compare(a, b);
}
// The function that generates the structure,
// by which the pivot table is rendered in ChartKit
// eslint-disable-next-line
function preparePivotTable({ placeholders, resultData, colors, sort, colorsConfig, idToTitle, idToDataType, ChartEditor, }) {
    const { data, order } = resultData;
    // Fields dragged into "columns"
    const c = placeholders[0].items;
    ...
    const hashTableCKeys = {};
    const colorHashTableCKeys = {};
    // Table "lowercase key" - "true/false"
    // (depending on whether there are values for this key)
    const hashTableRKeys = {};
    const colorHashTableRKeys = {};
    ...
    data.forEach((values) => {
        ...
        c.forEach((headColumnItem, level) => {
            ...
            if (typeof indices[actualTitle] === 'undefined') {
                i = indices[actualTitle] = (0, misc_helpers_1.findIndexInOrder)(order, headColumnItem, actualTitle);
            }
            else {
        ...
        r.forEach((headRowItem, level) => {
            ...
            let i;
            if (typeof indices[actualTitle] === 'undefined') {
                i = indices[actualTitle] = (0, misc_helpers_1.findIndexInOrder)(order, headRowItem, actualTitle);
            }
            else {
                i = indices[actualTitle];
            }
        ...
        m.forEach((measureItem) => {
            ...
            let i;
            if (typeof indices[actualTitle] === 'undefined') {
                i = indices[actualTitle] = (0, misc_helpers_1.findIndexInOrder)(order, measureItem, actualTitle);
            }
            else {
                i = indices[actualTitle];
            }
            ...
            if (value !== null) {
                if ((0, misc_helpers_1.isNumericalDataType)(dataType)) {
                    ...
                    formattedValue = (0, misc_helpers_1.chartKitFormatNumberWrapper)(value, {
                        lang: 'ru',
                        ...((_a = measureItem.formatting) !== null && _a !== void 0 ? _a : {
                            precision: (0, misc_helpers_1.isFloatDataType)(dataType) ? shared_1.MINIMUM_FRACTION_DIGITS : 0,
                        }),
                    });
                }
                else if ((0, shared_1.isDateField)(measureItem)) {
                    value = (0, misc_helpers_1.getTimezoneOffsettedTime)(new Date(value));
                }
            }
            // If necessary, you need to supplement the key with a pseudo-field "Measure Names"
            if (measureNamesExists || !rl || !cl) {
                let rPathSpecial = [...rPath];
                let cPathSpecial = [...cPath];
                ...
                if (colors.length) {
                    ...
                    if (typeof indices[actualColorTitle] === 'undefined') {
                        j = indices[actualColorTitle] = (0, misc_helpers_1.findIndexInOrder)(order, colors[0], actualColorTitle);
                    }
                    else {
        ...
        if (!multimeasure) {
            if (colors.length) {
                ...
                let i;
                if (typeof indices[actualColorTitle] === 'undefined') {
                    i = indices[actualColorTitle] = (0, misc_helpers_1.findIndexInOrder)(order, colors[0], actualColorTitle);
                }
                else {
    ...
    (0, misc_helpers_1.logTiming)('Inited', initTime);
    ...
    const flic = Object.keys(hashTableRKeys).length *
    ...
    (0, misc_helpers_1.log)('estimated result time:', estimatedResultTime);
    ...
    function generateHeadRows() {
        function generateHeadRowLevel(level, accumulated) {
            const localResult = [];
            accumulated.forEach((accumulatedRow) => {
                const keyPrefix = accumulatedRow.length
                    ...
                    : null;
                headRow[level].forEach((headRowLevel) => {
                    var _a;
                    ...
                    const currentKey = keyPrefix === null
                        ? originalValue
                    ...
                    else if ((0, shared_1.isDateField)({ data_type: rTypes[level] })) {
                    ...
                    else if ((0, misc_helpers_1.isNumericalDataType)(rTypes[level]) && ((_a = r[level]) === null || _a === void 0 ? void 0 : _a.formatting)) {
                        formattedValue = (0, misc_helpers_1.chartKitFormatNumberWrapper)(Number(headRowLevel), {
                            lang: 'ru',
                            ...r[level].formatting,
                        });
                    }
    ...
    function generateHeadColumns() {
        let headerId = 0;
        function generateHeadColumnLevel(level, key) {
            return (headColumn[level]
                // eslint-disable-next-line complexity
                ...
                if (!measureNamesInRow &&
                    ...
                    if ((0, misc_helpers_1.isNumericalDataType)(mType)) {
                        cell.type = 'number';
                        cell.sortable = true;
                        const mItemFormatting = mItem === null || mItem === void 0 ? void 0 : mItem.formatting;
                        if (mItemFormatting) {
                            cell.formatter = {
                                ...
                                precision: mType === shared_1.DATASET_FIELD_TYPES.FLOAT &&
                            ...
                            cell.precision =
                                mType === shared_1.DATASET_FIELD_TYPES.FLOAT
                                    ? shared_1.MINIMUM_FRACTION_DIGITS
                                    : 0;
                        }
                    }
                    else if ((0, shared_1.isDateField)({ data_type: mType })) {
                        cell.type = 'date';
                        cell.sortable = true;
                        cell.format = mItem.format;
                    }
                }
                if ((0, shared_1.isDateField)({ data_type: cType })) {
                    const dateLabel = (0, misc_helpers_1.formatDate)({
                        valueType: cType,
                        value: entry,
                ...
                else if ((0, shared_1.isMarkupField)({ data_type: cType })) {
                    cell.name = (0, shared_1.markupToRawString)(entry);
                    cell.markup = entry;
                }
                else if ((0, misc_helpers_1.isNumericalDataType)(cType) && ((_a = c[level]) === null || _a === void 0 ? void 0 : _a.formatting)) {
                    cell.name = entry === null ? 'null' : entry;
                    cell.formattedName = (0, misc_helpers_1.chartKitFormatNumberWrapper)(entry, {
                        lang: 'ru',
                        ...c[level].formatting,
                    });
                }
                ...
                const currentKey = typeof key === 'string'
                ...
                else if ((0, shared_1.isDateField)({ data_type: cType })) {
                    // 1.2.1 Двухуровневые заголовки: день месяца + день недели (для листовых колонок с датой)
                    const dayOfWeek = getDayOfWeekShort(entry);
                    if (dayOfWeek) {
                        cell.sub = [
                            { id: `${cell.id}-d`, name: cell.name, type: 'text' },
                            { id: `${cell.id}-dw`, name: dayOfWeek, type: 'text' },
                        ];
                    }
                }
                return cell;
    ...
    (0, misc_helpers_1.logTiming)('Generated rows', new Date() - startTime);
    const head = generateHeadColumns();
    (0, misc_helpers_1.logTiming)('Generated head', new Date() - startTime);
    let colorData;
    if (colors.length) {
        colorData = (0, color_helpers_1.mapAndColorizeHashTableByGradient)(colorHashTable, colorsConfig).colorData;
        (0, misc_helpers_1.logTiming)('Map and colorize', new Date() - startTime);
    }
    // The function of generating keys of existing data in the correct order
    function getPaths(structure, level, current, target, direction) {
        if (current.length) {
    ...
    (0, misc_helpers_1.logTiming)('getPaths headRow', new Date() - startTime);
    if (headRowPaths.length === 0) {
        headRowPaths = [['']];
    }
    let headColumnPaths = [];
    getPaths(headColumn, 0, [], headColumnPaths, 'column');
    (0, misc_helpers_1.logTiming)('getPaths headColumn', new Date() - startTime);
    ...
    headRowPaths.forEach((headRowPath) => {
        ...
        headColumnPaths.forEach((headColumnPath) => {
            ...
            else if ((0, misc_helpers_1.isNumericalDataType)(type)) {
                cell.type = 'number';
                cell.formattedValue = formattedValue;
            }
            else if ((0, shared_1.isDateField)({ data_type: type })) {
                cell.type = 'date';
            ...
            const measureName = measureNamesInRow
                ? ((_a = headRowPath[headRowPath.length - 1]) !== null && _a !== void 0 ? _a : '')
                : ((_b = headColumnPath[headColumnPath.length - 1]) !== null && _b !== void 0 ? _b : '');
            const measureByTitle = m.find((measure) => idToTitle[measure.guid] === measureName || measure.title === measureName);
            const preset = (_c = measureByTitle === null || measureByTitle === void 0 ? void 0 : measureByTitle.backgroundSettings) === null || _c === void 0 ? void 0 : _c.cellStylePreset;
            cell.custom = {
                trafficLightPercent: preset === 'trafficLight',
                turquoiseMeasure: preset === 'turquoise',
            };
            dataCells.push(cell);
        });
    ...
    // 1.2.2 Строка ИТОГО внизу (grand total): одна строка с итогами по всем строкам
    const numRowDimCols = rows.length > 0 && rows[0].cells.length >= headColumnPaths.length
        ? rows[0].cells.length - headColumnPaths.length
        : (headRow.length || 1);
    const footerCells = [];
    for (let i = 0; i < numRowDimCols; i++) {
        footerCells.push({
            value: i === 0 ? 'Итого' : null,
            id: ++cellId,
            type: 'text',
            name: i === 0 ? 'Итого' : '',
            css: { fontSize: '13px', lineHeight: '15px', fontWeight: 600 },
        });
    }
    headColumnPaths.forEach((headColumnPath) => {
        const cKey = headColumnPath.join(SPECIAL_DELIMITER);
        ...
        headRowPaths.forEach((headRowPath) => {
            ...
            const rKey = headRowPath.join(SPECIAL_DELIMITER);
            const key = `${cKey};${rKey}`;
            const value = (_a = hashTable[key]) === null || _a === void 0 ? void 0 : _a[0];
            const type = typeTable[key];
            if (value != null && (0, misc_helpers_1.isNumericalDataType)(type)) {
                const n = Number(value);
                if (!Number.isNaN(n)) {
                    sum = (sum !== null && sum !== void 0 ? sum : 0) + n;
                }
            }
        });
        const cell = { value: sum, id: ++cellId };
        if (sum != null) {
            cell.type = 'number';
            cell.precision = 2;
            cell.formattedValue = formattedSum !== null && formattedSum !== void 0 ? formattedSum : (typeof sum === 'number' ? String(sum) : undefined);
            cell.css = { fontSize: '13px', lineHeight: '15px', fontWeight: 600 };
        }
        else {
            cell.type = 'text';
            cell.css = { fontSize: '13px', lineHeight: '15px' };
        }
        footerCells.push(cell);
    });
    const footer = footerCells.length > 0 ? [{ cells: footerCells }] : undefined;
    (0, misc_helpers_1.logTiming)('Result', new Date() - startTime);
    return footer ? { head, rows, footer } : { head, rows };
}
exports.default = preparePivotTable;
