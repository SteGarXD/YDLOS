...
function checkLimit(metricasData) {
    ...
    if (limit < totalRows) {
        throw new Error('rows limit exceeded: ' + limit + '(limit) < ' + totalRows + '(total_rows)');
    }
}
/**
 * Normalize dimension
...
function buildSimpleStatFormatData(metrikasData) {
    ...
    const categories = prepareDatesMs(query.date1, query.date2, group && group[1] && group[1].toLowerCase());
    const graphs = {};
    const sequences = [];
