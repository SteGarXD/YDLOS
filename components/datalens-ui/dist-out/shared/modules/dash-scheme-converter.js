...
exports.migrateBgColor = migrateBgColor;
exports.preparedData = preparedData;
const date_utils_1 = require("@gravity-ui/date-utils");
const omitBy_1 = __importDefault(require("lodash/omitBy"));
const dash_1 = require("../constants/dash");
const widgets_1 = require("../constants/widgets");
const types_1 = require("../types");
const DATE_FORMAT_V7 = 'YYYY-MM-DD';
function getActualBackground(background) {
    if (background && widgets_1.DUPLICATED_WIDGET_BG_COLORS_PRESET.includes(background.color)) {
        return {
...
function migrateBgColor(item) {
    const newItem = Object.assign({ ...item }, { data: Object.assign({}, item.data) });
    if ('background' in newItem.data) {
        if (newItem.data.background &&
            widgets_1.DUPLICATED_WIDGET_BG_COLORS_PRESET.includes(newItem.data.background.color)) {
            newItem.data.background = getActualBackground(newItem.data.background);
            return newItem;
        }
    }
    if (newItem.type === types_1.DashTabItemType.Widget) {
...
function preparedData(data) {
    data.tabs.forEach((dashTabItem) => {
        dashTabItem.items = dashTabItem.items.map((wi) => {
            ...
            if (widgetItem.type !== types_1.DashTabItemType.Control) {
                return widgetItem;
            }
...
class DashSchemeConverter {
    static isUpdateNeeded(data) {
        return data.schemeVersion < dash_1.DASH_CURRENT_SCHEME_VERSION;
    }
    static update(data) {
        if (data.schemeVersion < dash_1.DASH_CURRENT_SCHEME_VERSION) {
    ...
    static async upTo3(data) {
        ...
        const convertedTabs = await Promise.all(tabs.map(async (tab) => {
            const { id: tabId, items, title, layout, ignores = [] } = tab;
            return {
                id: tabId,
                items: await Promise.all(items.map(async ({ id, data: itemData, tabs, type, defaults, namespace = 'default', }) => {
                    const data = itemData || tabs;
                    if (type === types_1.DashTabItemType.Control && !defaults) {
    ...
    static async upTo6(originalData) {
        ...
        const convertedTabs = tabs.map(({ ignores, ...tab }) => ({
            ...
            ignores: ignores.reduce((result, { who, whom }) => {
                tab.items
                    .filter(({ id, type }) => id === who && type === types_1.DashTabItemType.Widget)
                    .forEach(({ data }) => data.forEach(({ id }) => {
    ...
    static async upTo7(originalData) {
        ...
        tabs.forEach((tab) => {
            ...
            tab.items = tab.items.map((item) => {
                ...
                if (type === types_1.DashTabItemType.Control) {
                    const { title, control, dataset: { id: datasetId = null, fieldId: datasetFieldId = null } = {}, external: { entryId = null } = {}, showTitle, sourceType, } = item.data;
                    if (control.elementType === types_1.DashTabItemControlElementType.Date &&
                        typeof control.defaultValue === 'object') {
                        const { defaultValue: { type, value = {} }, } = control;
                        if (control.isRange) {
                            if (type === 'relative') {
                                const { from = '0', to = '0' } = value;
                                const resultFrom = from[0] === '-' ? `+${from.replace('-', '')}` : `-${from || 0}`;
                                const resultTo = to[0] === '-' ? `+${to.replace('-', '')}` : `-${to || 0}`;
                                control.defaultValue = `__interval___relative_${resultFrom}d___relative_${resultTo}d`;
                            }
                            else if (type === 'date' && value.from && value.to) {
                                control.defaultValue = `__interval_${(0, date_utils_1.dateTimeUtc)({ input: value.from }).format(DATE_FORMAT_V7)}_${(0, date_utils_1.dateTimeUtc)({ input: value.to }).format(DATE_FORMAT_V7)}`;
                            }
                            else {
                            ...
                            control.defaultValue = (0, date_utils_1.dateTimeUtc)({ input: value.from }).format(DATE_FORMAT_V7);
                        }
                        else {
                            control.defaultValue = '';
                        }
                    ...
                    if (control.elementType === types_1.DashTabItemControlElementType.Select &&
                        control.acceptableValues) {
                        control.acceptableValues = control.acceptableValues.map((value) => ({
                            title: value,
                            value,
                        }));
                    }
            ...
            tab.connections = tab.ignores.map(({ who, whom }) => ({
                from: who,
                to: whom,
                kind: types_1.DashTabConnectionKind.Ignore,
            }));
            delete tab.ignores;
        });
        const result = {
    ...
    static async upTo8(originalData) {
...
exports.default = DashSchemeConverter;
