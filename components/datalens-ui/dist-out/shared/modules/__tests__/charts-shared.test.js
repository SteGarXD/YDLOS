...
describe('charts-shared date helpers', () => {
    mockdate_1.default.set('2020-02-14T22:34:55.359Z');
    test.each([
        [null, null],
        [undefined, null],
        ['', null],
        ...
        const result = (0, charts_shared_1.resolveRelativeDate)(input);
    ...
    test.each([
        ...
        const result = (0, charts_shared_1.resolveIntervalDate)(input);
        expect(result).toEqual(expected);
    });
});
describe('charts-shared resolveOperation', () => {
    ...
    it('should parse value from url and return object {operation: Operations, value: string}', () => {
        const expectedResult = {
            operation: charts_shared_1.Operations.EQ,
            value: '2',
        };
        const result = (0, charts_shared_1.resolveOperation)(simpleUrlValue);
    ...
    it('should properly parse values with multiple underscores', () => {
        ...
        const result = (0, charts_shared_1.resolveOperation)(complexUrlValue);
        expect(result).toEqual(expectedResult);
    });
    it('should properly parse values with any characters', () => {
        const expectedResult = {
            operation: charts_shared_1.Operations.CONTAINS,
            value: 'symbols_~_[]_\\_|_()_*_&_^_%_$_#_№_@_"_!_;_:_{}_=',
        };
        const result = (0, charts_shared_1.resolveOperation)(symbolsInUrlValue);
        expect(result).toEqual(expectedResult);
    });
    it('should return null if input incoming value is casted to false', () => {
        ...
        const result = falsyValues.map((val) => (0, charts_shared_1.resolveOperation)(val));
        expect(result).toEqual(expectedResult);
    });
    it('should return the IN/BETWEEN default operation and the entered value, if the operator is not listed in Operations', () => {
        const expectedResult = {
            operation: charts_shared_1.Operations.IN,
            value: missedOperation,
        };
        const result = (0, charts_shared_1.resolveOperation)(missedOperation);
        expect(result).toEqual(expectedResult);
    });
    it('should return the IN/BETWEEN default operation and the entered value, if operation do not have value', () => {
        ...
        const result = (0, charts_shared_1.resolveOperation)(withoutValue);
        expect(result).toEqual(expectedResult);
    });
    it('should return the default BETWEEN operation if the operation has a value __interval', () => {
        ...
        const result = (0, charts_shared_1.resolveOperation)(intervalUrlValue);
        expect(result).toEqual(expectedResult);
    });
    it.each([
        ...Object.values(charts_shared_1.OperationsWithoutValue).map((operation) => [`__${operation}_`, operation]),
    ])('should return operation and value = undefined if operation does not imply value', (stringOperation, rawOperation) => {
        const result = (0, charts_shared_1.resolveOperation)(stringOperation);
        expect(result).toEqual({
            operation: rawOperation,
            value: undefined,
        });
    });
    it.each([[{ fake: 1 }], [1], [[]], [true]])('should return value with IN operation if value is not string', (value) => {
        const expectedValue = {
            operation: charts_shared_1.Operations.IN,
            value: value,
        };
        const result = (0, charts_shared_1.resolveOperation)(value);
...
describe('charts-shared splitParamsToParametersAndFilters', () => {
    ...
    it('should delete operations from parameters', () => {
        ...
        const result = (0, charts_shared_1.splitParamsToParametersAndFilters)(searchParams, parameterFields);
