...
export async function datalensTestSetup({
    ...
    try {
        if (isAuthDisabled) {
            ...
            const {login, password} = authSettings;
            if (
                login === undefined ||
                login === '' ||
                password === undefined ||
                ...
                throw new Error(
                    [
                        'E2E auth is enabled but credentials are missing.',
                        'Set E2E_USER_LOGIN and E2E_USER_PASSWORD (see tests/README.md — .env in project root),',
                        'or skip sign-in if your stack allows it: NO_AUTH=true, E2E_NO_AUTH=true, or AUTH_ENABLED=false.',
                    ].join(' '),
                );
            }
