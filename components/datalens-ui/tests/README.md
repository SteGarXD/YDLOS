...
```sh
E2E_DOMAIN=http://localhost:8080
# or e.g. npm run dev on 3031:
# E2E_DOMAIN=http://localhost:3031
```

If Playwright fails on `page.fill` with “expected string, got undefined”, you did not load `E2E_USER_LOGIN` / `E2E_USER_PASSWORD`. Add them to `.env` or disable auth for a local stack without sign-in: `NO_AUTH=true` (or `E2E_NO_AUTH=true`, or `AUTH_ENABLED=false`).

For `flatTableRowTreeMultiExpand.test.ts` against a **local dashboard** (not the CI wizard dump), set the entry path, e.g. `E2E_FLAT_TABLE_ROW_TREE_PAGE=/055mqd4bgqgsk-zagruzka-segmentov-reysov` (optional `?state=...`). See `documentation/env_configuration.md`.
