...
import {AUTHORIZATION_HEADER, DL_AUTH_HEADER_KEY, US_MASTER_TOKEN_HEADER} from '../../../const/common';
...
export const appAuth = async (req: Request, res: Response, next: NextFunction) => {
    ...
    // Запросы от UI через gateway с master-токеном (universalService до логина и т.п.)
    const masterTokens = req.ctx.config.masterToken;
    const requestMasterToken = req.headers[US_MASTER_TOKEN_HEADER] as string | undefined;
    if (
        Array.isArray(masterTokens) &&
        masterTokens.length > 0 &&
        requestMasterToken &&
        masterTokens.includes(requestMasterToken)
    ) {
        req.ctx.log('AUTH_MASTER_TOKEN');
        next();
        return;
    }
