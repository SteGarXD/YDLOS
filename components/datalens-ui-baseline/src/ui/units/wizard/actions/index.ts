    ...
    return async function (dispatch: WizardDispatch, getState: () => DatalensGlobalState) {
        ...
        return getDataset({id, workbookId})
            .then(async (dataset: Dataset) => {
                ...
                // Multi-dataset forces client-side pivot fallback only for pivot charts.
                const currentVizId = getState().wizard.visualization.visualization?.id;
                if (replacing !== true && datasets.length > 1 && currentVizId === 'pivotTable') {
