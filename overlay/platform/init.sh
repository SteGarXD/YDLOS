...
if [ "${IS_STOP}" == "true" ]; then
  ...
  docker --log-level error compose -f docker-compose.yaml -f docker-compose.own-images.yaml rm --stop --force
...
if [ "${IS_DOWN}" == "true" ]; then
  ...
  if [ "${IS_RM_VOLUMES}" == "true" ]; then
    ...
    if [ "${IS_DEV}" == "true" ]; then
      docker --log-level error compose -f docker-compose.yaml -f docker-compose.own-images.yaml down --remove-orphans --volumes
    else
...
if [ "${IS_DEV}" == "true" ]; then
  ...
  DOCKER_COMPOSE_DEV_CONFIG="docker-compose.yaml -f docker-compose.own-images.yaml"
  ...
  if [ "${IS_DEV_NGINX}" == "true" ]; then
    ...
    if [ ! -f "./certs/${DOMAIN}.crt" ]; then
      mkdir -p ./certs
      openssl req -x509 -batch -subj "/CN=${DOMAIN}/O=DataLens" -addext "subjectAltName=DNS:${DOMAIN}" -nodes -newkey rsa:2048 -sha256 -days 365 -keyout "./certs/${DOMAIN}.key" -out "./certs/${DOMAIN}.crt" &>/dev/null
      # shellcheck disable=SC2086
      rm -rf ./certs/${DOMAIN}.ca.crt && cat ./certs/${DOMAIN}.* >>./certs/${DOMAIN}.ca.crt
    fi
