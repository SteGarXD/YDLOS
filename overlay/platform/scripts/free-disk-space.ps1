...
# Не удаляет образы ydl/*, используемые текущим стеком.
...
docker rmi ydl/dl-ui:local 2>&1 | Out-Null
if ($LASTEXITCODE -eq 0) { Write-Host "Removed ydl/dl-ui:local" -ForegroundColor Green } else { Write-Host "Image not found or in use (skip)." -ForegroundColor Gray }
