...
export type DLGlobalData = {
    ...
    /** С сервера: запросы IAM resolveUsersByIds / getUsersByIds (на opensource по умолчанию выкл. без ZITADEL). */
    authResolveUsersByIdsEnabled?: boolean;
    /** Opensource: сервер разрешает обход NOT_AUTHENTICATED для сайдбара при живом RPC JWT (метаданные часто рассинхронизированы). */
    authAsideHeaderLandingBypassAllowed?: boolean;
    /** Значение US_MASTER_TOKEN с сервера — в клиентском бандле process.env недоступен. */
    usMasterToken?: string;
    exportDashExcel?: boolean;
    authManageLocalUsersDisabled?: boolean;
    authSignupDisabled?: boolean;
    /** Соответствует `FLIGHT_GROUPS_EDITOR_TOKEN` на сервере; нужен для заголовка `x-flight-groups-editor-token` в UI. */
    flightGroupsEditorToken?: string;
    /** Опционально: id из `FLIGHT_GROUPS_EDITOR_DATASET_IDS` (legacy, на кнопку не влияет). */
    flightGroupsEditorDatasetIds?: string[];
    /** Редактор доступен на сервере (MSSQL или FLIGHT_GROUPS_EDITOR_TOKEN). */
