...
function prepareArrayFilterValues({
    ...
    return values.reduce(
        (
            ...
            if (value !== null) {
                // YDL OS: мультивыбор дашборда часто приходит одной строкой "a,b,c" для IN —
                // разбиваем на отдельные значения, иначе SQL получает IN ('a,b,c') вместо IN ('a','b','c').
                if (
                    (operation === Operations.IN || operation === Operations.NIN) &&
                    typeof value === 'string' &&
                    !value.startsWith('__') &&
                    value.includes(',')
                ) {
