 ...
 */
/** Server-side «развернуть всё»: один ключ в treeState раскрывает все уровни иерархии. */
export const FLAT_TABLE_TREE_EXPAND_ALL = '__expand_all__';

const TREE_KEY_CHUNK = /\[[^\]]*\]/g;

export function isFlatTableTreeExpandAllState(raw: string[] | undefined): boolean {
    return normalizeFlatTableTreeStateList(raw).includes(FLAT_TABLE_TREE_EXPAND_ALL);
}
