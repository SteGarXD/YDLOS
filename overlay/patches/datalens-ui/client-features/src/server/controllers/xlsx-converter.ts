...
import {extractTwoHeaderRowsFromHead, headHasNestedSub} from './xlsx-pivot-layout';
...
type CellStyleMeta = {bg?: string | null; color?: string | null} | string | null;

function readCellStyleBg(meta: CellStyleMeta | undefined): string | null | undefined {
    if (!meta) {
        return null;
    }
...
function readCellStyleColor(meta: CellStyleMeta | undefined): string | null | undefined {
...
/** Сводная: серая шапка. Обычная таблица org-workbooks: синяя шапка как на экране. */
const PIVOT_HEADER_FILL_ARGB = 'FFD3D3D3';
const FLAT_TABLE_HEADER_FILL_ARGB = 'FF4A65A0';
const FLAT_TABLE_HEADER_TEXT_ARGB = 'FFFFFFFF';
...
export type ChartDataWithStyles = {
    ...
    graphs: Graph[];
    /** Параллель сетке таблицы: [строка][колонка] — bg/color из cell.css (export.js) */
    cellStyles?: CellStyleMeta[][];
    /** Сырое дерево шапки сводной (для 2 строк в Excel) */
    tableHead?: unknown;
    /** Стили колонок шапки обычной таблицы (из cell.css) */
    headerColumnStyles?: Array<{bg?: string | null; color?: string | null; align?: string | null}>;
    xlsxFooterMeta?: {
        bodyRowCount: number;
        footerRowCount: number;
        mergeFooterItogoLabel?: boolean;
    };
    preferFlatXlsxHeader?: boolean;
};
...
export function xlsxConverter(
    ...
    const run = async () => {
        ...
        if (templatePath) {
            ...
            const usePivotHeader =
                !chartData.preferFlatXlsxHeader &&
                headHasNestedSub(chartData.tableHead) &&
                Boolean(chartData.tableHead);
            const twoRows = usePivotHeader
                ? extractTwoHeaderRowsFromHead(chartData.tableHead, titleRow.length)
                : null;
            if (twoRows && twoRows.row1.length === titleRow.length) {
                ...
                titleRow.forEach((title, c) => {
                    ...
                    const colStyle = chartData.headerColumnStyles?.[c];
                    const headerBg =
                        cssBackgroundToArgb(colStyle?.bg) ?? FLAT_TABLE_HEADER_FILL_ARGB;
                    const headerFg =
                        cssBackgroundToArgb(colStyle?.color) ?? FLAT_TABLE_HEADER_TEXT_ARGB;
                    const headerAlign =
                        colStyle?.align === 'center'
                            ? 'center'
        ...
        const hasPartialCellStyles = Boolean(cellStyles?.length);
        ...
        rows.forEach((row, ri) => {
            ...
            row.forEach((cellVal, ci) => {
                ...
                if (!inFooter && typeof plain !== 'number') {
                ...
                const styleMeta = stylesAligned
                    ? cellStyles?.[ri]?.[ci] ?? null
                    : hasPartialCellStyles
                      ? cellStyles?.[ri]?.[ci] ?? null
                      : null;
                const bgCss = readCellStyleBg(styleMeta);
                const fgCss = readCellStyleColor(styleMeta);
                ...
                const fgArgb = cssBackgroundToArgb(fgCss);
                if (fgArgb) {
                    cell.font = {
                        ...(cell.font || {}),
                        color: {argb: fgArgb},
                        size: (cell.font as {size?: number})?.size ?? 11,
                    };
                }
