...
const VISUALIZATION_SECTIONS = [
    ...
    ['pie', 'donut'],
    ['metric', 'indicator'],
    ['treemap', 'flatTable', 'pivotTable'],
];

function getVisualizationSection(id: string) {
    const index = VISUALIZATION_SECTIONS.findIndex((section) => section.includes(id));
    return index === -1 ? VISUALIZATION_SECTIONS.length : index;
}
...
class VisualizationSelector extends React.Component<Props> {
    ...
    private getItems(): DropdownMenuItemMixed<unknown>[] {
        ...
        const grouped = visibleVisualizations.reduce<Record<number, DropdownMenuItemMixed<unknown>[]>>(
            (acc, item, index) => {
                const section = getVisualizationSection(item.id);
        ...
        return Object.keys(grouped)
            .map(Number)
            .sort((a, b) => a - b)
            .reduce<DropdownMenuItemMixed<unknown>[]>((items, section) => {
                const group = grouped[section];
                if (group?.length) {
                    items.push(group as DropdownMenuItemMixed<unknown>);
                }
                return items;
            }, []);
    }
