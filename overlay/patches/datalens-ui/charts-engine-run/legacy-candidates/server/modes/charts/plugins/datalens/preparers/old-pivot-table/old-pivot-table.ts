...
const DAY_OF_WEEK_SHORT = ['вс', 'пн', 'вт', 'ср', 'чт', 'пт', 'сб'] as const;

/** 1.2.1 Двухуровневые заголовки: день недели (пн, вт, …) для колонок с датой */
function getDayOfWeekShort(value: string | number | Date | null): string {
    if (value === null || value === undefined) return '';
    const m = moment(value);
    return m.isValid() ? DAY_OF_WEEK_SHORT[m.day()] ?? '' : '';
}
...
function preparePivotTable({
            ...
            return (
                headColumn[level]
                    ...
                    .map((entry: any, entryIndex: number) => {
                        ...
                        } else if (isDateField({data_type: cType})) {
    ...
    headRowPaths.forEach((headRowPath) => {
        ...
        headColumnPaths.forEach((headColumnPath) => {
            ...
            const measureName = measureNamesInRow
                ? headRowPath[headRowPath.length - 1] ?? ''
                : headColumnPath[headColumnPath.length - 1] ?? '';
            const measureByTitle = m.find(
                (measure) =>
                    idToTitle[measure.guid] === measureName || measure.title === measureName,
            );
            const preset = measureByTitle?.backgroundSettings?.cellStylePreset;
            cell.custom = {
    ...
    const numRowDimCols =
        rows.length > 0 && rows[0].cells.length >= headColumnPaths.length
    ...
    headColumnPaths.forEach((headColumnPath) => {
        ...
        let formattedSum: string | undefined;
        headRowPaths.forEach((headRowPath) => {
            const rKey = headRowPath.join(SPECIAL_DELIMITER);
            const key = `${cKey};${rKey}`;
            const value = hashTable[key]?.[0];
            const type = typeTable[key];
            if (value !== null && value !== undefined && isNumericalDataType(type)) {
        ...
        const cell: any = {value: sum, id: ++cellId};
        if (sum !== null && sum !== undefined) {
            cell.type = 'number';
            cell.precision = 2;
            cell.formattedValue =
                formattedSum ?? (typeof sum === 'number' ? String(sum) : undefined);
