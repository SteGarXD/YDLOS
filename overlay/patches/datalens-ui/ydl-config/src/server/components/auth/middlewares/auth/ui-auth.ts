...
import {onAuthLogout, onAuthSignin} from '../../callbacks';
import type {AccessTokenPayload} from '../../types/token';

import {ALGORITHMS} from './constants';

const clearAuthCookies = (res: Response, authCookieName: string, authExpCookieName: string) => {
    // Stale auth cookies can trap the browser in /auth/reload -> 409 when refresh token is gone.
    res.clearCookie(authCookieName, {path: '/'});
    res.clearCookie(authExpCookieName, {path: '/'});
};
...
export const uiAuth = async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (now + ACCESS_TOKEN_TIME_RESERVE > exp) {
        ...
        try {
            ...
            clearAuthCookies(res, authCookieName, authExpCookieName);
            req.ctx.log('SIGNIN_AFTER_REFRESH_TOKEN_ERROR');
            onAuthSignin(req, res);
            return;
        }
    }
